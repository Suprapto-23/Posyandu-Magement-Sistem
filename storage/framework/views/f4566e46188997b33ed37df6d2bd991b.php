...
<?php $__env->startSection('title', 'Detail Profil Bidan'); ?>
<?php $__env->startSection('page-name', 'Profil Bidan'); ?>
    ...
    <div class="bg-gradient-to-br from-sky-500 to-teal-500 rounded-[2.5rem] p-8 md:p-12 mb-8 relative overflow-hidden shadow-lg border border-white/20 flex flex-col items-center justify-center text-center group">
        ...
        <div class="relative z-10 w-full flex flex-col items-center">
            ...
            <h2 class="text-3xl md:text-4xl font-black text-white font-poppins tracking-tight text-shadow-sm">
                <?php echo e($bidan->profile->full_name ?? $bidan->name); ?>
            ...
            <div class="mt-4 flex flex-wrap items-center justify-center gap-3">
                ...
                <span class="bg-white/20 backdrop-blur-md border border-white/30 text-white font-mono text-[11px] font-bold px-4 py-1.5 rounded-full flex items-center gap-2 shadow-sm">
                    <i class="fas fa-id-card"></i> NIK: <?php echo e($bidan->nik ?? $bidan->profile?->nik ?? '-'); ?>
            ...
            <div class="mt-8 flex gap-3">
                <a href="<?php echo e(route('admin.bidans.index')); ?>" class="bg-white/20 hover:bg-white/30 backdrop-blur-sm border border-white/30 text-white text-[11px] uppercase tracking-widest font-bold px-6 py-3 rounded-xl transition-all smooth-route"><i class="fas fa-arrow-left mr-1"></i> Kembali</a>
                <a href="<?php echo e(route('admin.bidans.edit', $bidan->id)); ?>" class="bg-white hover:bg-slate-50 text-teal-600 text-[11px] uppercase tracking-widest font-bold px-6 py-3 rounded-xl transition-all shadow-lg hover:-translate-y-0.5 smooth-route"><i class="fas fa-edit mr-1"></i> Edit Profil</a>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 hover:shadow-md hover:border-slate-200 transition-all">
            ...
            <div class="space-y-4">
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    ...
                    <span class="text-[13px] font-black text-slate-700"><?php echo e(($bidan->profile?->jenis_kelamin == 'L') ? 'Laki-Laki' : (($bidan->profile?->jenis_kelamin == 'P') ? 'Perempuan' : '-')); ?></span>
                </div>
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Tempat Lahir</span>
                    <span class="text-[13px] font-black text-slate-700"><?php echo e($bidan->profile?->tempat_lahir ?? '-'); ?></span>
                </div>
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Tanggal Lahir</span>
                    <span class="text-[13px] font-black text-slate-700"><?php echo e($bidan->profile?->tanggal_lahir ? \Carbon\Carbon::parse($bidan->profile->tanggal_lahir)->translatedFormat('d F Y') : '-'); ?></span>
                </div>
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Usia</span>
                    <span class="text-[13px] font-black text-teal-600 bg-teal-50 px-2 py-0.5 rounded"><?php echo e($bidan->profile?->tanggal_lahir ? \Carbon\Carbon::parse($bidan->profile->tanggal_lahir)->age . ' Tahun' : '-'); ?></span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 hover:shadow-md hover:border-slate-200 transition-all">
            ...
            <div class="space-y-4">
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    ...
                    <span class="text-[13px] font-black text-slate-700 break-all"><?php echo e($bidan->email); ?></span>
                </div>
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Telepon / WhatsApp</span>
                    <span class="text-[13px] font-black text-slate-700"><?php echo e($bidan->profile?->telepon ?? '-'); ?></span>
                </div>
                <div class="flex flex-col border-b border-slate-50 pb-3 gap-1">
                    ...
                    <span class="text-[13px] font-black text-slate-700 leading-relaxed"><?php echo e($bidan->profile?->alamat ?? '-'); ?></span>
                </div>
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Terakhir Login</span>
                    <span class="text-[13px] font-black text-slate-500"><?php echo e($bidan->last_login_at ? \Carbon\Carbon::parse($bidan->last_login_at)->translatedFormat('d M Y, H:i') : 'Belum Pernah'); ?></span>
                </div>
            </div>
        </div>
    </div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\POSYANDU\posyandu-management-system\resources\views/admin/bidans/show.blade.php ENDPATH**/ ?>
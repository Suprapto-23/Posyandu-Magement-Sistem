            ...
            <table class="w-full text-left border-collapse min-w-[1000px]">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="table-row-hover bg-white group">
                        <td class="px-6 py-5 text-sm font-black text-slate-400 align-top"><?php echo e($jadwals->firstItem() + $index); ?></td>
                        ...
                        <td class="px-6 py-5 align-top">
                            <div class="flex items-start gap-3">
                                <div class="w-12 h-12 rounded-xl bg-slate-50 border border-slate-200 flex flex-col items-center justify-center shrink-0 shadow-sm">
                                    <span class="text-[9px] font-black text-cyan-600 uppercase"><?php echo e(\Carbon\Carbon::parse($jadwal->tanggal)->translatedFormat('M')); ?></span>
                                    <span class="text-[18px] font-black text-slate-800 leading-none"><?php echo e(\Carbon\Carbon::parse($jadwal->tanggal)->format('d')); ?></span>
                                </div>
                                <div>
                                    <p class="font-black text-slate-800 text-[14px] mb-1.5"><?php echo e(\Carbon\Carbon::parse($jadwal->tanggal)->translatedFormat('l, Y')); ?></p>
                                    <p class="text-[11px] font-bold text-slate-500 bg-slate-100 inline-block px-2 py-0.5 rounded-md border border-slate-200 shadow-sm">
                                        <i class="fas fa-clock text-cyan-500 mr-1"></i> <?php echo e(date('H:i', strtotime($jadwal->waktu_mulai))); ?> - <?php echo e(date('H:i', strtotime($jadwal->waktu_selesai))); ?>
                        ...
                        <td class="px-6 py-5 align-top">
                            <p class="font-black text-slate-800 text-[15px] mb-1.5 text-wrap"><?php echo e($jadwal->judul); ?></p>
                            <p class="text-[12px] font-medium text-slate-500 line-clamp-2 mb-2 leading-relaxed"><?php echo e($jadwal->deskripsi ?? 'Tidak ada deskripsi tambahan untuk kegiatan ini.'); ?></p>
                        ...
                        <td class="px-6 py-5 text-center align-middle">
                            ...
                            ?>
                            <span class="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border shadow-sm <?php echo e($statusConf[0]); ?>">
                        ...
                        <td class="px-6 py-5 text-right align-middle">
                            <div class="flex items-center justify-end gap-2">
                                <a href="<?php echo e(route('bidan.jadwal.edit', $jadwal->id)); ?>" class="smooth-route w-10 h-10 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center hover:bg-amber-500 hover:text-white hover:shadow-md transition-all border border-amber-100 hover:border-amber-500" title="Edit Jadwal">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="<?php echo e(route('bidan.jadwal.destroy', $jadwal->id)); ?>" method="POST" onsubmit="return confirm('PERINGATAN: Menghapus jadwal akan menghilangkan data ini dari HP Warga secara permanen. Lanjutkan?');" class="m-0 p-0">

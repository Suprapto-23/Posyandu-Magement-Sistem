...
<body x-data="{ sidebarOpen: window.innerWidth >= 1280 }" @resize.window="sidebarOpen = window.innerWidth >= 1280" class="flex h-screen overflow-hidden selection:bg-cyan-100 selection:text-cyan-900">
    ...
    <div class="flex-1 flex flex-col min-w-0 h-screen relative transition-all duration-300 ease-in-out"
            ...
            <div class="flex items-center gap-3 sm:gap-5">
                ...
                <div x-data="{ openNotif: false }" class="relative">
                    ...
                    <div x-show="openNotif" x-cloak
                        ...
                        <div class="px-5 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
                            ...
                            <?php if($notifCount > 0): ?>
                                <span class="bg-rose-100 text-rose-600 text-[9px] font-black px-2.5 py-1 rounded-lg uppercase tracking-widest"><?php echo e($notifCount); ?> Menunggu</span>
                            <?php endif; ?>
                        </div>

                        <div class="max-h-[320px] overflow-y-auto custom-scrollbar bg-white">
                            <?php $__empty_1 = true; $__currentLoopData = $pendingNotifs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $notif): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <?php
                                ...
                                ?>
                                <a href="<?php echo e(route('bidan.pemeriksaan.show', $notif->id)); ?>" class="flex items-start gap-4 p-4 border-b border-slate-50 hover:bg-cyan-50 transition-colors group">
                                    <div class="flex-1 min-w-0">
                                        <p class="text-[13px] font-bold text-slate-800 truncate group-hover:text-cyan-600"><?php echo e($namaPasien); ?></p>
                                        <div class="flex items-center gap-2 mt-0.5">
                                            <span class="text-[9px] font-black text-cyan-500 uppercase"><?php echo e($kat); ?></span>
                                            <span class="text-[10px] text-slate-400"><?php echo e($notif->created_at->diffForHumans()); ?></span>
                                        </div>
                                    </div>
                ...
                <div x-data="{ openProfile: false }" class="relative">
                    <button @click="openProfile = !openProfile" @click.away="openProfile = false" class="flex items-center gap-2.5 pl-1.5 pr-4 py-1.5 rounded-full bg-white border border-slate-200 shadow-sm hover:bg-slate-50 transition-colors focus:outline-none">
                        ...
                        <span class="hidden md:block text-[13px] font-bold text-slate-700 truncate max-w-[120px]"><?php echo e(Str::words(Auth::user()->name, 2, '')); ?></span>
        ...
        <nav class="xl:hidden fixed bottom-0 left-0 right-0 h-[68px] glass-panel border-t border-slate-200 z-50 flex items-center justify-around px-2 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
            ...
            <div class="relative w-full h-full flex justify-center">
                <a href="<?php echo e(route('bidan.pemeriksaan.index')); ?>" class="absolute -top-5 flex flex-col items-center justify-center w-14 h-14 rounded-full bg-gradient-to-tr from-cyan-500 to-blue-600 text-white shadow-[0_8px_20px_rgba(6,182,212,0.4)] border-4 border-white transition-transform active:scale-95 hover:-translate-y-1">
    ...
    <script>
        ...
        <?php if(session('success')): ?> showToast('success', 'Berhasil!', "<?php echo e(session('success')); ?>"); <?php endif; ?>
        <?php if(session('error')): ?> showToast('error', 'Peringatan!', "<?php echo e(session('error')); ?>"); <?php endif; ?>
    </script>

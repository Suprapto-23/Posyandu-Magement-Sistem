...
<div class="max-w-[1400px] mx-auto pb-10">
    ...
    <div class="animate-slide-up bg-gradient-to-br from-cyan-600 via-cyan-700 to-blue-800 rounded-[32px] p-8 md:p-10 mb-8 relative overflow-hidden shadow-[0_15px_40px_rgba(6,182,212,0.3)] border border-cyan-400 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div class="relative z-10 flex items-center gap-6">
            ...
            <div>
                <span class="inline-flex items-center gap-2 px-3 py-1 bg-cyan-900/50 text-[10px] font-black uppercase tracking-widest rounded-lg mb-2 backdrop-blur-md border border-cyan-500/30 text-white">
                    <i class="far fa-calendar-alt text-cyan-400"></i> <?php echo e(\Carbon\Carbon::now()->translatedFormat('l, d F Y')); ?>

                </span>
                <h1 class="text-2xl sm:text-3xl font-black text-white tracking-tight leading-none mb-1 font-poppins">Selamat Bertugas, <?php echo e(Auth::user()->name ?? 'Bidan'); ?>!</h1>
        ...
        <div class="relative z-10 hidden sm:block">
            <a href="<?php echo e(route('bidan.pemeriksaan.index')); ?>" class="smooth-route inline-flex items-center justify-center gap-2 px-6 py-4 bg-white text-cyan-700 font-black text-[12px] uppercase tracking-widest rounded-xl hover:bg-cyan-50 transition-all shadow-[0_10px_20px_rgba(0,0,0,0.1)] hover:-translate-y-1">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8 animate-slide-up-delay-1">
        ...
        <div class="stat-card bg-white rounded-[24px] p-6 border border-slate-200/80 shadow-sm relative overflow-hidden group">
            ...
            <div>
                <h3 class="text-[32px] font-black text-slate-800 font-poppins leading-none mb-1"><?php echo e($antrianPending); ?></h3>
        ...
        <div class="stat-card bg-white rounded-[24px] p-6 border border-slate-200/80 shadow-sm relative overflow-hidden group">
            ...
            <div>
                <h3 class="text-[32px] font-black text-slate-800 font-poppins leading-none mb-1"><?php echo e($totalPasien); ?></h3>
        ...
        <div class="stat-card bg-white rounded-[24px] p-6 border border-slate-200/80 shadow-sm relative overflow-hidden group">
            ...
            <div>
                <h3 class="text-[32px] font-black text-slate-800 font-poppins leading-none mb-1"><?php echo e($totalImunisasi); ?></h3>
        ...
        <div class="stat-card bg-white rounded-[24px] p-6 border border-slate-200/80 shadow-sm relative overflow-hidden group">
            ...
            <div>
                <h3 class="text-[32px] font-black text-slate-800 font-poppins leading-none mb-1"><?php echo e($jadwalAktif); ?></h3>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 animate-slide-up-delay-2">
        ...
        <div class="xl:col-span-1 bg-white rounded-[32px] border border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.03)] p-6 md:p-8 flex flex-col">
            ...
            <div class="flex-1 flex flex-col gap-4">
                <?php $__empty_1 = true; $__currentLoopData = $upcomingJadwals; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $jadwal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="bg-slate-50/80 rounded-2xl p-4 border border-slate-100 hover:border-indigo-200 transition-colors group relative overflow-hidden">
                        <div class="absolute left-0 top-0 bottom-0 w-1 bg-<?php echo e($jadwal->target_peserta == 'ibu_hamil' ? 'pink' : 'indigo'); ?>-500 rounded-l-2xl"></div>
                        <div class="flex justify-between items-start mb-1">
                            <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest"><?php echo e(\Carbon\Carbon::parse($jadwal->tanggal)->translatedFormat('d M Y')); ?></p>
                            <span class="text-[8px] font-black px-2 py-0.5 rounded border shadow-sm <?php echo e($jadwal->target_peserta == 'ibu_hamil' ? 'bg-pink-50 text-pink-600 border-pink-200' : 'bg-indigo-50 text-indigo-600 border-indigo-200'); ?>">
                                <?php echo e($jadwal->label_target); ?>

                            </span>
                        </div>
                        <h4 class="text-[13px] font-bold text-slate-800 font-poppins mb-2 line-clamp-1"><?php echo e($jadwal->judul); ?></h4>
                        <div class="flex items-center gap-3 text-[11px] font-bold text-slate-500">
                            <span class="flex items-center gap-1 bg-white px-2 py-0.5 rounded border border-slate-200 shadow-sm"><i class="far fa-clock text-cyan-500"></i> <?php echo e(date('H:i', strtotime($jadwal->waktu_mulai))); ?></span>
                            <span class="flex items-center gap-1 bg-white px-2 py-0.5 rounded border border-slate-200 shadow-sm"><i class="fas fa-map-marker-alt text-rose-400"></i> <?php echo e(Str::limit($jadwal->lokasi, 15)); ?></span>
                        </div>
                    </div>
            ...
            <a href="<?php echo e(route('bidan.jadwal.index')); ?>" class="smooth-route mt-6 w-full text-center py-3 bg-indigo-50 text-indigo-600 font-black text-[11px] uppercase tracking-widest rounded-xl hover:bg-indigo-600 hover:text-white transition-all duration-300">
                Kelola Semua Jadwal
            </a>
        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if(ctx) {
            ...
            const countBalita = <?php echo e(\App\Models\Balita::count() ?? 0); ?>;
            const countBumil = <?php echo e(\App\Models\IbuHamil::count() ?? 0); ?>;
            const countRemaja = <?php echo e(\App\Models\Remaja::count() ?? 0); ?>;
            const countLansia = <?php echo e(\App\Models\Lansia::count() ?? 0); ?>;

...
<?php $__env->startSection('page-name', 'Koreksi Profil Remaja'); ?>
...
<style>
    ...
    /* FORM INPUT CRM NEXUS (TEMA INDIGO) */
...
<div id="smoothLoader" class="fixed inset-0 bg-slate-50/90 backdrop-blur-md z-[9999] flex flex-col items-center justify-center transition-all duration-300 opacity-100 pointer-events-auto">
    <div class="relative w-20 h-20 flex items-center justify-center mb-4">
        ...
        <div class="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-lg"><i class="fas fa-pen-nib text-indigo-600 text-2xl animate-pulse"></i></div>
    </div>
    <p class="text-indigo-900 font-black tracking-widest text-[11px] animate-pulse uppercase">MENYIAPKAN DATA...</p>
</div>
...
<div class="max-w-6xl mx-auto animate-slide-up relative z-10 pb-12">
    ...
    <div class="mb-6 flex items-center gap-3 relative z-10">
        <a href="<?php echo e(route('kader.data.remaja.index')); ?>" onclick="showLoader()" class="w-12 h-12 rounded-[16px] bg-white border border-slate-200 text-slate-500 flex items-center justify-center hover:bg-indigo-50 hover:border-indigo-300 hover:text-indigo-600 transition-all shadow-sm group">
    ...
    <div class="text-center mb-10 relative z-10">
        ...
        <div class="inline-flex items-center gap-2 bg-indigo-50 border border-indigo-200 text-indigo-600 text-[9px] font-black px-3 py-1 rounded-full mb-3 uppercase tracking-widest mx-auto block w-max">
            <i class="fas fa-exclamation-circle text-indigo-400"></i> Mode Koreksi Data Aktif
        </div>
        <h1 class="text-3xl font-black text-slate-900 tracking-tight font-poppins">Edit Profil Remaja</h1>
        <p class="text-slate-500 mt-2 font-medium text-[13px] max-w-lg mx-auto">Pembaruan NIK pada modul ini akan berdampak pada akses login portal Warga milik peserta remaja tersebut.</p>
    </div>

    
    <form action="<?php echo e(route('kader.data.remaja.update', $remaja->id)); ?>" method="POST" id="formEditRemaja" class="relative z-10">
            ...
            <div class="lg:col-span-7 glass-panel rounded-[32px] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.04)] p-8 md:p-10 relative overflow-hidden flex flex-col">
                ...
                <div class="space-y-6 flex-1">
                    <div>
                        ...
                        <input type="number" name="nik" value="<?php echo e(old('nik', $remaja->nik)); ?>" required class="form-input focus:ring-4 focus:ring-indigo-50 <?php $__errorArgs = ['nik'];
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_lengkap" value="<?php echo e(old('nama_lengkap', $remaja->nama_lengkap)); ?>" required class="form-input focus:ring-4 focus:ring-indigo-50">
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            <label class="form-label">Tempat Lahir <span class="text-rose-500">*</span></label>
                            <input type="text" name="tempat_lahir" value="<?php echo e(old('tempat_lahir', $remaja->tempat_lahir)); ?>" required class="form-input focus:ring-4 focus:ring-indigo-50">
                        </div>
                        <div>
                            <label class="form-label">Tanggal Lahir <span class="text-rose-500">*</span></label>
                            <input type="date" id="tanggal_lahir" name="tanggal_lahir" value="<?php echo e(old('tanggal_lahir', $remaja->tanggal_lahir?->format('Y-m-d'))); ?>" required class="form-input cursor-pointer focus:ring-4 focus:ring-indigo-50" onchange="calculateAge()">
                    ...
                    <div>
                        ...
                        <textarea name="alamat" rows="2" required class="form-input resize-none focus:ring-4 focus:ring-indigo-50"><?php echo e(old('alamat', $remaja->alamat)); ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-5 flex flex-col gap-8">
                ...
                <div class="bg-indigo-50/80 rounded-[32px] border border-indigo-100 shadow-[0_10px_40px_-10px_rgba(79,70,229,0.03)] p-8 md:p-10 relative overflow-hidden">
                    ...
                    <div class="space-y-6 relative z-10">
                        <div class="p-4 bg-white border border-indigo-100 rounded-2xl shadow-sm">
                            ...
                            <input type="text" name="sekolah" value="<?php echo e(old('sekolah', $remaja->sekolah)); ?>" class="form-input bg-slate-50 focus:bg-white focus:border-indigo-400 mb-4">
                            
                            <label class="form-label text-indigo-600"><i class="fas fa-chalkboard mr-1"></i> Tingkat Kelas</label>
                            <input type="text" name="kelas" value="<?php echo e(old('kelas', $remaja->kelas)); ?>" class="form-input bg-slate-50 focus:bg-white focus:border-indigo-400">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-[32px] border border-slate-200 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.03)] p-8 md:p-10 relative overflow-hidden flex-1 flex flex-col">
                    <div class="flex items-center gap-4 mb-8 border-b border-slate-100 pb-5">
                        <span class="w-10 h-10 rounded-[14px] bg-blue-500 text-white flex items-center justify-center font-black shadow-md">3</span>
                    ...
                    <div class="space-y-6 flex-1">
                        <div>
                            ...
                            <input type="text" name="nama_ortu" value="<?php echo e(old('nama_ortu', $remaja->nama_ortu)); ?>" required class="form-input focus:ring-4 focus:ring-indigo-50">
                        </div>
                        <div>
                            <label class="form-label">No. HP Keluarga (WhatsApp)</label>
                            <input type="number" name="telepon_ortu" value="<?php echo e(old('telepon_ortu', $remaja->telepon_ortu)); ?>" class="form-input focus:ring-4 focus:ring-indigo-50">
                        </div>
                        <div>
                            <label class="form-label">Golongan Darah</label>
                            <select name="golongan_darah" class="form-input cursor-pointer focus:ring-4 focus:ring-indigo-50">
                                ...
                                <?php $__currentLoopData = ['A','B','AB','O','A+','A-','B+','B-','AB+','AB-','O+','O-']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $gol): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($gol); ?>" <?php echo e(old('golongan_darah', $remaja->golongan_darah) == $gol ? 'selected' : ''); ?>><?php echo e($gol); ?></option>
        ...
        <div class="mt-8 bg-white border border-slate-200 p-6 md:p-8 rounded-[32px] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] flex flex-col sm:flex-row items-center justify-between gap-6 relative z-30">
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 rounded-[16px] bg-indigo-50 text-indigo-500 flex items-center justify-center text-2xl shrink-0"><i class="fas fa-edit"></i></div>
                <div class="hidden sm:block">
                    <h4 class="text-[14px] font-black text-slate-800 font-poppins mb-0.5">Simpan Perubahan</h4>
                    <p class="text-[12px] font-medium text-slate-500 leading-relaxed">Data NIK dan profil akan diperbarui ke seluruh jaringan database warga.</p>
                </div>
            </div>
            <div class="flex items-center gap-3 w-full sm:w-auto shrink-0">
                <a href="<?php echo e(route('kader.data.remaja.index')); ?>" onclick="showLoader()" class="flex-1 sm:flex-none px-8 py-4 bg-slate-100 border border-slate-200 text-slate-600 font-extrabold text-[12px] rounded-full hover:bg-slate-200 transition-colors text-center uppercase tracking-widest">
                    Batalkan
                </a>
                <button type="submit" id="btnSubmit" class="flex-1 sm:flex-none px-10 py-4 bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-black text-[12px] rounded-full hover:from-indigo-700 hover:to-blue-700 shadow-[0_8px_20px_rgba(79,70,229,0.3)] hover:-translate-y-1 transition-all flex items-center justify-center gap-2 uppercase tracking-widest">
...
<script>
    ...
    function calculateAge() {
        ...
        if(months < 0) { 
            helper.innerHTML = '<div class="mt-2 text-rose-500 text-xs font-bold"><i class="fas fa-exclamation-triangle"></i> Tanggal di masa depan</div>'; 
            return; 
        }
        ...
        if(y < 10 || y > 19) {
            ...
            alertMsg = '<span class="text-[9px] bg-rose-500 text-white px-2 py-0.5 rounded ml-2 shadow-sm uppercase tracking-widest">Di Luar Rentang Remaja</span>';
        }
    ...
    <?php if(session('error')): ?>
        Swal.fire({
            ...
            customClass: { 
                ...
                confirmButton: 'bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3.5 rounded-full font-black text-[11px] uppercase tracking-widest transition-all shadow-md mt-4' 
            }
        });
    <?php endif; ?>
</script>

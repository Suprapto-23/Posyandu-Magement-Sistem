...
<?php $__env->startSection('title', 'Logbook Kunjungan'); ?>
<?php $__env->startSection('page-name', 'Buku Induk Kunjungan'); ?>
...
<div class="max-w-[1400px] mx-auto animate-slide-up pb-10">
    ...
    <div class="glass-card rounded-[24px] p-3 mb-6 flex flex-col xl:flex-row items-center justify-between gap-4 relative z-20">
        ...
        <form id="filterForm" action="<?php echo e(route('kader.kunjungan.index')); ?>" method="GET" class="w-full xl:w-auto">
            <input type="hidden" name="search" id="hiddenSearch" value="<?php echo e(request('search')); ?>">
            <input type="hidden" name="kategori" id="hiddenKategori" value="<?php echo e(request('kategori', 'semua')); ?>">
        </form>
        ...
        <div class="flex gap-2 w-full xl:w-auto overflow-x-auto custom-scrollbar pb-2 sm:pb-0 justify-start sm:justify-center bg-slate-100/50 p-1.5 rounded-[20px] border border-slate-200/50">
            ...
            <?php $__currentLoopData = $tabs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <button type="button" data-kategori="<?php echo e($k); ?>" class="tab-btn px-5 py-2.5 rounded-xl font-extrabold text-[11px] uppercase tracking-wider whitespace-nowrap transition-all flex items-center gap-2 border <?php echo e($currentCat == $k ? 'bg-cyan-600 text-white border-cyan-600 shadow-md active-tab' : 'bg-transparent text-slate-500 border-transparent hover:bg-white hover:border-slate-200 hover:shadow-sm'); ?>">
                   <?php if($v['icon']): ?> <i class="fas <?php echo e($v['icon']); ?> <?php echo e($currentCat == $k ? 'text-cyan-200' : 'text-slate-400'); ?>"></i> <?php endif; ?>
                   <?php echo e($v['label']); ?>
        ...
        <div class="w-full xl:w-96 relative group">
            ...
            <input type="text" id="liveSearchInput" value="<?php echo e(request('search')); ?>" placeholder="Cari riwayat nama warga..." class="search-input" autocomplete="off">
    ...
    <div id="mainContentArea">
        <div class="bg-white rounded-[24px] border border-slate-200 shadow-[0_5px_20px_-10px_rgba(0,0,0,0.05)] overflow-hidden" id="tableContainer">
                ...
                <table class="w-full text-left border-collapse min-w-[900px]">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors group">
                            ...
                            <td class="px-6 py-4 border-r border-slate-100">
                                <div class="flex items-center gap-3">
                                    <div class="w-12 h-12 rounded-[14px] bg-slate-100 flex flex-col items-center justify-center text-slate-600 border border-slate-200 shrink-0 group-hover:bg-cyan-50 group-hover:text-cyan-600 transition-colors">
                                        <span class="text-[10px] font-bold uppercase tracking-widest leading-none mb-0.5"><?php echo e(\Carbon\Carbon::parse($kunjungan->created_at)->translatedFormat('M')); ?></span>
                                        <span class="text-[16px] font-black leading-none font-poppins"><?php echo e(\Carbon\Carbon::parse($kunjungan->created_at)->format('d')); ?></span>
                                    </div>
                                    <div>
                                        <p class="text-[12px] font-bold text-slate-500"><i class="far fa-clock"></i> <?php echo e($kunjungan->created_at->format('H:i')); ?> WIB</p>
                                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5"><?php echo e($kunjungan->created_at->diffForHumans()); ?></p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 border-r border-slate-100">
                                <p class="font-black text-slate-800 text-[14px] mb-1.5 truncate max-w-[200px] font-poppins group-hover:text-cyan-700 transition-colors"><?php echo e($kunjungan->pasien->nama_lengkap ?? 'Data Terhapus'); ?></p>
                                <?php 
                                ...
                                ?>
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border shadow-sm <?php echo e($badge); ?>">
                                    <i class="fas fa-tag"></i> <?php echo e(match($tipe) { 'IbuHamil' => 'Ibu Hamil', default => $tipe }); ?>
                            ...
                            <td class="px-6 py-4 border-r border-slate-100">
                                <div class="flex flex-wrap gap-2 mb-2">
                                    ...
                                    <?php endif; ?>
                                    <?php if($kunjungan->imunisasis && $kunjungan->imunisasis->count() > 0): ?>
                                    ...
                                    <?php endif; ?>
                                    <?php if(!$kunjungan->pemeriksaan && (!$kunjungan->imunisasis || $kunjungan->imunisasis->count() == 0)): ?>
                                ...
                                <?php if($kunjungan->keluhan): ?>
                                    <p class="text-[10px] font-bold text-slate-500 italic max-w-[200px] truncate"><i class="fas fa-quote-left text-slate-300 mr-1"></i> <?php echo e($kunjungan->keluhan); ?></p>
                                <?php else: ?>
                            ...
                            <td class="px-6 py-4 text-center border-r border-slate-100">
                                <div class="inline-flex flex-col items-center justify-center">
                                    <div class="w-8 h-8 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400 mb-1.5">
                                        <?php echo e(strtoupper(substr($kunjungan->petugas->name ?? 'K', 0, 1))); ?>

                                    </div>
                                    <span class="text-[11px] font-bold text-slate-700"><?php echo e($kunjungan->petugas->name ?? 'Sistem Otomatis'); ?></span>
                                </div>
                            </td>

                            
                            <td class="px-6 py-4 text-center">
                                <a href="<?php echo e(route('kader.kunjungan.show', $kunjungan->id)); ?>" class="loader-trigger inline-flex w-10 h-10 rounded-[12px] bg-white border border-slate-200 items-center justify-center text-slate-400 hover:text-cyan-600 hover:border-cyan-300 hover:bg-cyan-50 shadow-sm transition-all hover:scale-105" title="Buka Arsip Kunjungan">
            ...
            <div id="paginationArea" class="px-6 py-4 border-t border-slate-100 bg-slate-50/80">
                <?php if(isset($kunjungans) && $kunjungans->hasPages()): ?> <?php echo e($kunjungans->links()); ?> <?php endif; ?>
            </div>
        </div>
    </div>
</div>
...
<?php echo $__env->make('layouts.kader', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\POSYANDU\posyandu-management-system\resources\views/kader/kunjungan/index.blade.php ENDPATH**/ ?>
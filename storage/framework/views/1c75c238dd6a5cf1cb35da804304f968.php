...
<div class="max-w-[1400px] mx-auto animate-slide-up">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-8">
        ...
        <div class="flex items-center gap-3">
            <a href="<?php echo e(route('kader.import.index')); ?>" class="px-5 py-2.5 bg-emerald-50 text-emerald-600 font-extrabold text-sm rounded-xl hover:bg-emerald-100 transition-colors border border-emerald-200 shadow-sm">
                <i class="fas fa-file-excel mr-1.5"></i> Import Excel
            </a>
            <a href="<?php echo e(route('kader.data.remaja.create')); ?>" class="px-5 py-2.5 bg-indigo-600 text-white font-extrabold text-sm rounded-xl hover:bg-indigo-700 shadow-[0_4px_12px_rgba(79,70,229,0.3)] hover:-translate-y-0.5 transition-all">
    ...
    <div class="glass-card rounded-3xl p-4 mb-6 flex flex-col xl:flex-row items-center gap-4 justify-between">
        ...
        <div class="flex items-center gap-4 w-full xl:w-auto">
            <div class="flex items-center gap-3 px-4 py-2 bg-indigo-50 border border-indigo-100 rounded-2xl">
                ...
                <div>
                    ...
                    <p class="text-lg font-black text-indigo-700 leading-none"><?php echo e($remajas->total()); ?> <span class="text-xs font-bold text-indigo-500">Anak</span></p>
                </div>
            </div>
            
            
            <form action="<?php echo e(route('kader.data.remaja.bulk-delete')); ?>" method="POST" id="bulkDeleteForm" class="hidden">
                <?php echo csrf_field(); ?>
        ...
        <form action="<?php echo e(route('kader.data.remaja.index')); ?>" method="GET" class="w-full xl:w-1/3 relative group">
            <input type="text" name="search" id="liveSearchInput" value="<?php echo e(request('search')); ?>" placeholder="Cari Nama, NIK, atau Sekolah..." 
                ...
                <table class="w-full text-left border-collapse whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-indigo-50/30 transition-colors">
                            <td class="px-4 py-4 text-center"><input type="checkbox" name="ids[]" value="<?php echo e($item->id); ?>" class="checkbox-modern row-checkbox" onchange="checkBulkStatus()"></td>
                            ...
                            <td class="px-4 py-4">
                                <p class="text-xs font-bold text-slate-700 mb-1 flex items-center gap-1.5"><i class="fas fa-school text-indigo-400"></i> <?php echo e($item->sekolah ?? 'Tidak Sekolah'); ?> <?php echo $item->kelas ? "<span class='text-slate-400'>| Kls: {$item->kelas}</span>" : ''; ?></p>
                                <p class="text-[11px] font-bold text-slate-500"><i class="fas fa-map-marker-alt text-rose-400 w-3 text-center"></i> <?php echo e($item->tempat_lahir); ?>, <?php echo e(\Carbon\Carbon::parse($item->tanggal_lahir)->translatedFormat('d M Y')); ?></p>
                            </td>

                            
                            <td class="px-4 py-4">
                                <p class="text-xs font-bold text-slate-800 mb-0.5"><i class="fas fa-user-friends text-emerald-400 w-4"></i> <?php echo e($item->nama_ortu ?? 'Data Ortu Kosong'); ?></p>
                                <?php if($item->telepon_ortu): ?>
                                    <a href="https://wa.me/<?php echo e(preg_replace('/[^0-9]/', '', $item->telepon_ortu)); ?>" target="_blank" class="inline-flex items-center gap-1 text-[11px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200 hover:bg-emerald-100 transition-colors">
                                        <i class="fab fa-whatsapp"></i> <?php echo e($item->telepon_ortu); ?>
                            ...
                            <td class="px-4 py-4 text-center">
                                <span class="block mb-2 w-max mx-auto px-3 py-1 rounded-lg text-[11px] font-black bg-indigo-100 text-indigo-700 border border-indigo-200 shadow-sm transition-transform hover:scale-105">
                                    <i class="fas fa-birthday-cake text-indigo-400 mr-1"></i> <?php echo e($umur); ?> TAHUN
                                </span>
                                <?php if($item->user_id): ?>
                                ...
                                <?php else: ?>
                                    <a href="<?php echo e(route('kader.data.remaja.sync', $item->id)); ?>" class="text-[10px] font-black text-amber-600 bg-amber-50 px-2 py-1 rounded-md border border-amber-200 hover:bg-amber-100 transition-colors"><i class="fas fa-sync"></i> Tarik Akun</a>
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="px-4 py-4 pr-6">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="<?php echo e(route('kader.data.remaja.show', $item->id)); ?>" class="w-9 h-9 rounded-xl bg-white border border-slate-200 text-indigo-500 hover:bg-indigo-50 hover:border-indigo-300 flex items-center justify-center transition-all shadow-sm" title="Lihat Rekam Medis"><i class="fas fa-file-medical"></i></a>
                                    <a href="<?php echo e(route('kader.data.remaja.edit', $item->id)); ?>" class="w-9 h-9 rounded-xl bg-white border border-slate-200 text-amber-500 hover:bg-amber-50 hover:border-amber-300 flex items-center justify-center transition-all shadow-sm" title="Edit Data"><i class="fas fa-pen"></i></a>
                                    
                                    <form action="<?php echo e(route('kader.data.remaja.destroy', $item->id)); ?>" method="POST" id="delete-form-<?php echo e($item->id); ?>">

...
<div class="max-w-7xl mx-auto animate-slide-up">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <a href="<?php echo e(route('kader.data.remaja.create')); ?>" class="inline-flex items-center justify-center gap-2 px-6 py-3.5 bg-indigo-600 text-white font-extrabold text-sm rounded-xl hover:bg-indigo-700 shadow-[0_4px_12px_rgba(79,70,229,0.3)] hover:-translate-y-0.5 transition-all">
    ...
    <div class="bg-white rounded-[24px] border border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-4 mb-6">
        <form action="<?php echo e(route('kader.data.remaja.index')); ?>" method="GET" class="flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1">
                <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-sm"></i>
                <input type="text" name="search" value="<?php echo e($search); ?>" placeholder="Cari nama remaja, NIK, atau sekolah..." class="search-input">
            </div>
            <div class="flex gap-2">
                ...
                <?php if($search): ?>
                    <a href="<?php echo e(route('kader.data.remaja.index')); ?>" class="px-6 py-3 bg-rose-50 text-rose-600 font-extrabold text-sm rounded-xl hover:bg-rose-100 transition-colors border border-rose-100 flex items-center">
                        Reset
                    </a>
                <?php endif; ?>
            </div>
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg shadow-sm border border-white <?php echo e($remaja->jenis_kelamin == 'L' ? 'bg-sky-100 text-sky-600' : 'bg-rose-100 text-rose-600'); ?>">
                                    <?php echo e(strtoupper(substr($remaja->nama_lengkap, 0, 1))); ?>

                                </div>
                                <div>
                                    <p class="font-extrabold text-slate-800 text-sm mb-0.5"><?php echo e($remaja->nama_lengkap); ?></p>
                                    <p class="text-[11px] font-bold text-slate-400 flex items-center gap-1.5">
                                        <i class="fas fa-barcode"></i> <?php echo e($remaja->nik); ?>
                        ...
                        <td class="px-6 py-4">
                            <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-100 text-slate-700 text-xs font-bold rounded-lg mb-1 border border-slate-200">
                                ...
                                <?php echo e(\Carbon\Carbon::parse($remaja->tanggal_lahir)->age); ?> Tahun
                            </div>
                            <p class="text-[11px] font-bold text-slate-500 pl-1"><i class="fas fa-phone-alt text-slate-400 mr-1"></i> <?php echo e($remaja->telepon_ortu ?? 'Belum ada No HP'); ?></p>
                        </td>

                        <td class="px-6 py-4">
                            <p class="font-bold text-slate-800 text-sm mb-0.5"><?php echo e($remaja->sekolah ?? '-'); ?></p>
                            <p class="text-[11px] font-bold text-slate-400">Kelas: <span class="text-indigo-600"><?php echo e($remaja->kelas ?? '-'); ?></span></p>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                <a href="<?php echo e(route('kader.data.remaja.show', $remaja->id)); ?>" class="w-9 h-9 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-500 hover:text-indigo-600 hover:border-indigo-300 hover:bg-indigo-50 shadow-sm transition-all" title="Detail Profil">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="<?php echo e(route('kader.data.remaja.edit', $remaja->id)); ?>" class="w-9 h-9 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-500 hover:text-amber-600 hover:border-amber-300 hover:bg-amber-50 shadow-sm transition-all" title="Edit Data">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="<?php echo e(route('kader.data.remaja.destroy', $remaja->id)); ?>" method="POST" onsubmit="return confirm('Tindakan ini tidak dapat dibatalkan. Yakin hapus data remaja?');" class="inline-block">
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-16 text-center">
                            ...
                            <a href="<?php echo e(route('kader.data.remaja.create')); ?>" class="inline-flex items-center gap-2 mt-4 text-indigo-600 font-bold hover:text-indigo-700">

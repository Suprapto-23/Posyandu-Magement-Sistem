...
<?php $__env->startSection('title', 'Detail Warga'); ?>
<?php $__env->startSection('page-name', 'Detail Profil Warga'); ?>
    ...
    <div class="bg-gradient-to-br from-obsidian-900 to-slate-800 rounded-[32px] p-8 md:p-12 mb-8 relative overflow-hidden shadow-xl border border-slate-700 flex flex-col items-center justify-center text-center group">
        ...
        <div class="relative z-10 w-full flex flex-col items-center">
            ...
            <div class="w-24 h-24 rounded-full bg-obsidian-900 border-2 border-amber-500 text-amber-500 flex items-center justify-center font-black text-4xl shadow-[0_0_25px_rgba(245,158,11,0.4)] mb-5">
                <?php echo e(strtoupper(substr($user->profile->full_name ?? $user->name, 0, 1))); ?>

            </div>
            
            <h2 class="text-3xl font-black text-white font-poppins tracking-tight">
                <?php echo e($user->profile->full_name ?? $user->name); ?>
            ...
            <div class="mt-3 flex items-center justify-center gap-3">
                <span class="bg-white/10 backdrop-blur-md border border-white/20 text-slate-200 font-mono text-xs font-bold px-4 py-1.5 rounded-lg flex items-center gap-2">
                    <i class="fas fa-id-card text-amber-500"></i> NIK: <?php echo e($user->nik ?? $user->profile?->nik ?? '-'); ?>
            ...
            <div class="mt-8 flex gap-3">
                <a href="<?php echo e(route('admin.users.index')); ?>" class="bg-slate-800 hover:bg-slate-700 border border-slate-600 text-white text-xs font-bold px-5 py-2.5 rounded-xl transition-all smooth-route"><i class="fas fa-arrow-left mr-1"></i> Kembali</a>
                <a href="<?php echo e(route('admin.users.edit', $user->id)); ?>" class="bg-amber-500 hover:bg-amber-400 text-obsidian-900 text-xs font-bold px-5 py-2.5 rounded-xl transition-all shadow-[0_4px_15px_rgba(245,158,11,0.3)] smooth-route"><i class="fas fa-edit mr-1"></i> Edit Profil</a>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white rounded-[32px] border border-slate-200 shadow-sm p-8 hover:shadow-md hover:border-slate-300 transition-all">
            ...
            <div class="space-y-4">
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    ...
                    <span class="text-sm font-bold text-slate-800"><?php echo e(($user->profile?->jenis_kelamin == 'L') ? 'Laki-Laki' : (($user->profile?->jenis_kelamin == 'P') ? 'Perempuan' : '-')); ?></span>
                </div>
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Tempat Lahir</span>
                    <span class="text-sm font-bold text-slate-800"><?php echo e($user->profile?->tempat_lahir ?? '-'); ?></span>
                </div>
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Tanggal Lahir</span>
                    <span class="text-sm font-bold text-slate-800"><?php echo e($user->profile?->tanggal_lahir ? \Carbon\Carbon::parse($user->profile->tanggal_lahir)->translatedFormat('d F Y') : '-'); ?></span>
                </div>
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Usia</span>
                    <span class="text-sm font-black text-amber-600"><?php echo e($user->profile?->tanggal_lahir ? \Carbon\Carbon::parse($user->profile->tanggal_lahir)->age . ' Tahun' : '-'); ?></span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[32px] border border-slate-200 shadow-sm p-8 hover:shadow-md hover:border-slate-300 transition-all">
            ...
            <div class="space-y-4">
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    ...
                    <span class="text-sm font-bold text-slate-800 break-all"><?php echo e($user->email); ?></span>
                </div>
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Telepon / WhatsApp</span>
                    <span class="text-sm font-bold text-slate-800"><?php echo e($user->profile?->telepon ?? '-'); ?></span>
                </div>
                <div class="flex flex-col border-b border-slate-50 pb-3 gap-1">
                    ...
                    <span class="text-sm font-bold text-slate-800 leading-relaxed"><?php echo e($user->profile?->alamat ?? '-'); ?></span>
                </div>
                <div class="flex flex-col sm:flex-row sm:justify-between border-b border-slate-50 pb-3 gap-1">
                    <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Akun Dibuat</span>
                    <span class="text-sm font-bold text-slate-800"><?php echo e($user->created_at->translatedFormat('d F Y, H:i')); ?></span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="mt-12 mb-10">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6 px-2">
            <div>
                ...
                <p class="text-[13px] font-medium text-slate-500 mt-1">Data rekam medis yang berafiliasi dengan NIK <?php echo e($user->profile->full_name ?? $user->name); ?>.</p>
            </div>
        </div>

        
        <?php if($linkedData['balita']->isEmpty() && empty($linkedData['remaja']) && empty($linkedData['lansia'])): ?>
        ...
        <?php else: ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <?php $__currentLoopData = $linkedData['balita']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $b): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="bg-white rounded-[24px] border border-slate-200 p-6 shadow-sm hover:shadow-xl hover:border-amber-300 transition-all duration-300 group flex items-center gap-5">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <h4 class="text-base font-bold text-obsidian-900 truncate"><?php echo e($b->nama_lengkap); ?></h4>
                        <div class="flex items-center gap-3 mt-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wide">
                            <span class="flex items-center gap-1"><i class="fas fa-id-card"></i> <?php echo e($b->nik ?? '-'); ?></span>
                            <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                            <span class="flex items-center gap-1"><i class="fas fa-clock"></i> <?php echo e(\Carbon\Carbon::parse($b->tanggal_lahir)->age); ?> Tahun</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-[24px] border border-slate-200 p-6 shadow-sm hover:shadow-xl hover:border-amber-300 transition-all duration-300 group flex items-center gap-5">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <h4 class="text-base font-bold text-obsidian-900 truncate"><?php echo e($linkedData['remaja']->nama_lengkap); ?></h4>
                        <div class="flex items-center gap-3 mt-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wide">
                            <span class="flex items-center gap-1"><i class="fas fa-id-card"></i> <?php echo e($linkedData['remaja']->nik ?? '-'); ?></span>
                            <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                            <span class="flex items-center gap-1"><i class="fas fa-clock"></i> <?php echo e(\Carbon\Carbon::parse($linkedData['remaja']->tanggal_lahir)->age); ?> Tahun</span>
                        </div>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <div class="bg-white rounded-[24px] border border-slate-200 p-6 shadow-sm hover:shadow-xl hover:border-amber-300 transition-all duration-300 group flex items-center gap-5">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <h4 class="text-base font-bold text-obsidian-900 truncate"><?php echo e($linkedData['lansia']->nama_lengkap); ?></h4>
                        <div class="flex items-center gap-3 mt-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wide">
                            <span class="flex items-center gap-1"><i class="fas fa-id-card"></i> <?php echo e($linkedData['lansia']->nik ?? '-'); ?></span>
                            <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                            <span class="flex items-center gap-1"><i class="fas fa-clock"></i> <?php echo e(\Carbon\Carbon::parse($linkedData['lansia']->tanggal_lahir)->age); ?> Tahun</span>
                        </div>
                    </div>
                </div>
                <?php endif; ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\POSYANDU\posyandu-management-system\resources\views/admin/users/show.blade.php ENDPATH**/ ?>
...
<div class="max-w-[1400px] mx-auto animate-slide-up">
    ...
    <div class="glass-card rounded-[24px] p-3 mb-6 flex flex-col xl:flex-row items-center gap-4 justify-between relative z-20">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 w-full xl:w-auto">
            <form action="<?php echo e(route('kader.data.ibu-hamil.index')); ?>" method="GET" class="flex gap-2 w-full sm:w-max">
                <input type="hidden" name="search" value="<?php echo e(request('search')); ?>">
                <div class="bg-slate-100/80 p-1.5 rounded-full flex w-full border border-slate-200/50 shadow-inner">
                    <button type="submit" name="filter" value="semua" class="flex-1 sm:flex-none px-6 py-2 rounded-full font-extrabold text-xs uppercase tracking-wider transition-all <?php echo e(request('filter', 'semua') == 'semua' ? 'bg-white text-pink-600 shadow-sm' : 'text-slate-500 hover:bg-slate-200/50'); ?>">Semua</button>
                    <button type="submit" name="filter" value="aktif" class="flex-1 sm:flex-none px-6 py-2 rounded-full font-extrabold text-xs uppercase tracking-wider transition-all <?php echo e(request('filter') == 'aktif' ? 'bg-gradient-to-r from-pink-500 to-rose-500 text-white shadow-md' : 'text-slate-500 hover:bg-slate-200/50'); ?>">Aktif</button>
                    <button type="submit" name="filter" value="hampir_lahir" class="flex-1 sm:flex-none px-6 py-2 rounded-full font-extrabold text-xs uppercase tracking-wider transition-all <?php echo e(request('filter') == 'hampir_lahir' ? 'bg-amber-500 text-white shadow-md' : 'text-slate-500 hover:bg-slate-200/50'); ?>"><i class="fas fa-clock mr-1"></i> HPL Dekat</button>
                </div>
            </form>
        ...
        <div class="table-container overflow-x-auto max-h-[65vh]">
            <table class="w-full text-left border-collapse whitespace-nowrap min-w-[1000px]">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-pink-50/40 transition-colors pasien-row" data-search="<?php echo e(strtolower($item->nama_lengkap . ' ' . $item->nik . ' ' . $item->nama_suami)); ?>">
                        <td class="px-5 py-4 text-center border-r border-slate-200/50"><input type="checkbox" name="ids[]" value="<?php echo e($item->id); ?>" class="checkbox-modern row-checkbox" onchange="checkBulkStatus()"></td>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-[10px] font-bold text-slate-500 font-mono tracking-wide bg-slate-100 px-2 py-0.5 rounded"><i class="far fa-id-card text-slate-400"></i> <?php echo e($item->nik ?? '-'); ?></span>
                                        <span class="text-[9px] font-black tracking-widest px-2 py-0.5 rounded border bg-sky-50 text-sky-600 border-sky-200" title="Suami">
                                            <i class="fas fa-male"></i> <?php echo e($item->nama_suami ?? '-'); ?>
                            ...
                            <div class="flex flex-col gap-1.5">
                                <div class="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[11px] font-bold text-slate-700"><?php echo e($item->hpht ? $item->hpht->translatedFormat('d M Y') : 'Belum diisi'); ?></p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[11px] font-black text-pink-700"><?php echo e($item->hpl ? $item->hpl->translatedFormat('d M Y') : 'Belum diisi'); ?></p>
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-5 py-4 border-r border-slate-200/50">
                            <div class="flex items-center gap-2 mb-1.5">
                                <span class="text-[10px] font-bold text-slate-500 bg-white border border-slate-200 px-2 py-1 rounded shadow-sm w-16 text-center">BB: <span class="text-indigo-600 font-black"><?php echo e($item->berat_badan ?? '-'); ?></span></span>
                                <span class="text-[10px] font-bold text-slate-500 bg-white border border-slate-200 px-2 py-1 rounded shadow-sm w-16 text-center">TB: <span class="text-emerald-600 font-black"><?php echo e($item->tinggi_badan ?? '-'); ?></span></span>
                            </div>
                            <?php
                            ...
                            ?>
                            <span class="inline-flex items-center gap-1 text-[9px] font-black uppercase tracking-widest border px-2 py-0.5 rounded <?php echo e($imtClass); ?>">
                                IMT: <?php echo e($item->imt ?? '-'); ?> (<?php echo e($imtLabel); ?>)
                            </span>
                        </td>
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <a href="<?php echo e(route('kader.data.ibu-hamil.show', $item->id)); ?>" class="w-9 h-9 rounded-xl bg-white border border-slate-200 text-indigo-500 hover:bg-indigo-50 hover:text-indigo-600 flex items-center justify-center transition-all shadow-sm hover:shadow-md" title="Buku KIA Digital"><i class="fas fa-book-medical"></i></a>
                                <a href="<?php echo e(route('kader.data.ibu-hamil.edit', $item->id)); ?>" class="w-9 h-9 rounded-xl bg-white border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-800 flex items-center justify-center transition-all shadow-sm hover:shadow-md" title="Edit Master Data"><i class="fas fa-edit"></i></a>
                                
                                <form action="<?php echo e(route('kader.data.ibu-hamil.destroy', $item->id)); ?>" method="POST" id="delete-form-<?php echo e($item->id); ?>">

...
<div class="p-4 md:p-8 font-poppins bg-[#f8fafc] min-h-screen">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <div class="shrink-0">
            <a href="<?php echo e(route('user.konseling.index')); ?>" class="inline-flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white text-xs font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200">
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6 relative overflow-hidden">
                ...
                <div class="relative z-10 flex flex-col items-center mt-6">
                    ...
                    <h2 class="text-center text-lg font-black text-slate-800"><?php echo e($remaja->nama_lengkap ?? $remaja->nama ?? 'Nama Remaja'); ?></h2>
                    <p class="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest"><?php echo e($remaja->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan'); ?></p>
                </div>
                ...
                <div class="mt-8 space-y-4">
                    <div class="flex justify-between items-center pb-3 border-b border-slate-50">
                        ...
                        <span class="text-sm font-bold text-slate-800"><?php echo e($remaja->nik ?? '-'); ?></span>
                    </div>
                    <div class="flex justify-between items-center pb-3 border-b border-slate-50">
                        <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Tgl Lahir</span>
                        <span class="text-sm font-bold text-slate-800"><?php echo e($remaja->tanggal_lahir ? \Carbon\Carbon::parse($remaja->tanggal_lahir)->format('d M Y') : '-'); ?></span>
                    </div>
                    <div class="flex justify-between items-center pb-3 border-b border-slate-50">
                        ...
                        <span class="text-sm font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded">
                            <?php echo e($remaja->tanggal_lahir ? \Carbon\Carbon::parse($remaja->tanggal_lahir)->age . ' Tahun' : '-'); ?>
        ...
        <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6 lg:p-8">
                    <div class="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p class="text-xs font-medium text-slate-400 mt-1">Divalidasi Bidan pada: <?php echo e(\Carbon\Carbon::parse($pemeriksaanTerakhir->tanggal_periksa)->translatedFormat('d F Y')); ?></p>
                        </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100 text-center">
                            ...
                            <h3 class="text-xl font-black text-slate-800"><?php echo e($pemeriksaanTerakhir->berat_badan ?? '-'); ?><span class="text-xs text-slate-500 ml-1">kg</span></h3>
                        </div>
                        <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100 text-center">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Tinggi</p>
                            <h3 class="text-xl font-black text-slate-800"><?php echo e($pemeriksaanTerakhir->tinggi_badan ?? '-'); ?><span class="text-xs text-slate-500 ml-1">cm</span></h3>
                        </div>
                        <div class="bg-violet-50 rounded-2xl p-4 border border-violet-100 text-center">
                            ...
                            <h3 class="text-xl font-black text-violet-700"><?php echo e($pemeriksaanTerakhir->tekanan_darah ?? '-'); ?></h3>
                        </div>
                        <div class="bg-rose-50 rounded-2xl p-4 border border-rose-100 text-center">
                            ...
                            <h3 class="text-xl font-black text-rose-600"><?php echo e($pemeriksaanTerakhir->hemoglobin ?? $pemeriksaanTerakhir->hb ?? '-'); ?></h3>
                        </div>
                    </div>

                    <?php if($pemeriksaanTerakhir->keterangan): ?>
                        <div class="mt-6 p-4 bg-indigo-50/50 rounded-xl border border-indigo-100">
                            ...
                            <p class="text-xs font-bold text-slate-700 leading-relaxed italic">"<?php echo e($pemeriksaanTerakhir->keterangan); ?>"</p>
                        </div>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
            ...
            <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6 lg:p-8">
                ...
                <div class="overflow-x-auto custom-scrollbar">
                    <?php if(isset($riwayatPemeriksaan) && $riwayatPemeriksaan->isNotEmpty()): ?>
                        <table class="w-full text-left min-w-[500px]">
                            ...
                            <tbody>
                                <?php $__currentLoopData = $riwayatPemeriksaan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $riwayat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr class="hover:bg-slate-50 transition-colors">
                                        <td class="py-4 px-2 border-b border-slate-50 text-sm font-bold text-slate-700">
                                            <?php echo e(\Carbon\Carbon::parse($riwayat->tanggal_periksa)->format('d/m/Y')); ?>

                                        </td>
                                        <td class="py-4 px-2 border-b border-slate-50 text-center text-xs font-bold text-slate-600">
                                            <?php echo e($riwayat->berat_badan ?? '-'); ?>kg / <?php echo e($riwayat->tinggi_badan ?? '-'); ?>cm
                                        </td>
                                        <td class="py-4 px-2 border-b border-slate-50 text-center text-xs font-bold text-slate-600">
                                            <?php echo e($riwayat->tekanan_darah ?? '-'); ?>

                                        </td>
                                        <td class="py-4 px-2 border-b border-slate-50 text-center text-xs font-bold text-rose-500">
                                            <?php echo e($riwayat->hemoglobin ?? $riwayat->hb ?? '-'); ?>

                                        </td>
                                        <td class="py-4 px-2 border-b border-slate-50">
                                            <?php if($riwayat->status_gizi): ?>
                                                <span class="px-2 py-1 bg-slate-100 text-slate-600 text-[10px] font-black uppercase rounded"><?php echo e($riwayat->status_gizi); ?></span>
                                            <?php else: ?>
                                                -
                                            <?php endif; ?>
                                        </td>

...
<div class="max-w-6xl mx-auto space-y-8">
    ...
    <div class="bg-gradient-to-br from-blue-600 to-sky-400 rounded-[2.5rem] p-10 relative overflow-hidden shadow-[0_20px_40px_-10px_rgba(59,130,246,0.3)] border border-white/20 flex flex-col items-center justify-center text-center group animate-pop-in">
        ...
        <div class="relative z-10">
            ...
            <a href="<?php echo e(route('admin.users.create')); ?>" class="inline-flex items-center gap-2 bg-white hover:bg-slate-50 text-blue-600 font-black px-7 py-3.5 rounded-xl transition-all shadow-lg hover:shadow-xl hover:-translate-y-1 smooth-route">
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-[1.5rem] p-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 animate-pop-in delay-100">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs font-medium text-blue-600">Berikan password ini kepada: <strong class="text-blue-800"><?php echo e(session('user_name') ?? session('reset_name')); ?></strong> (NIK: <?php echo e(session('user_nik') ?? session('reset_nik')); ?>)</p>
            </div>
        </div>
        <div class="bg-white border border-blue-200 rounded-xl px-5 py-3 flex items-center gap-4 shadow-sm">
            <code class="text-xl font-mono font-black text-blue-600 tracking-wider" id="passwordText"><?php echo e(session('generated_password') ?? session('reset_password')); ?></code>
    ...
    <div class="flex flex-wrap items-center gap-2.5 animate-pop-in delay-200">
        ...
        <a href="<?php echo e(route('admin.users.index')); ?>" class="px-5 py-2.5 rounded-full text-xs font-black tracking-wide transition-all duration-300 <?php echo e(!request('kategori') ? 'bg-blue-600 text-white shadow-md shadow-blue-500/30' : 'bg-white text-slate-500 hover:bg-blue-50 hover:text-blue-600 border border-slate-200'); ?>">
            Semua Warga
        </a>
        <a href="<?php echo e(route('admin.users.index', ['kategori' => 'balita', 'search' => request('search')])); ?>" class="px-5 py-2.5 rounded-full text-xs font-black tracking-wide transition-all duration-300 flex items-center gap-2 <?php echo e(request('kategori') == 'balita' ? 'bg-sky-500 text-white shadow-md shadow-sky-500/30' : 'bg-white text-slate-500 hover:bg-sky-50 hover:text-sky-600 border border-slate-200'); ?>">
            <i class="fas fa-baby text-sm"></i> Orang Tua Balita
        </a>
        <a href="<?php echo e(route('admin.users.index', ['kategori' => 'bumil', 'search' => request('search')])); ?>" class="px-5 py-2.5 rounded-full text-xs font-black tracking-wide transition-all duration-300 flex items-center gap-2 <?php echo e(request('kategori') == 'bumil' ? 'bg-pink-500 text-white shadow-md shadow-pink-500/30' : 'bg-white text-slate-500 hover:bg-pink-50 hover:text-pink-600 border border-slate-200'); ?>">
            <i class="fas fa-female text-sm"></i> Ibu Hamil
        </a>
        <a href="<?php echo e(route('admin.users.index', ['kategori' => 'remaja', 'search' => request('search')])); ?>" class="px-5 py-2.5 rounded-full text-xs font-black tracking-wide transition-all duration-300 flex items-center gap-2 <?php echo e(request('kategori') == 'remaja' ? 'bg-indigo-500 text-white shadow-md shadow-indigo-500/30' : 'bg-white text-slate-500 hover:bg-indigo-50 hover:text-indigo-600 border border-slate-200'); ?>">
            <i class="fas fa-user-graduate text-sm"></i> Remaja
        </a>
        <a href="<?php echo e(route('admin.users.index', ['kategori' => 'lansia', 'search' => request('search')])); ?>" class="px-5 py-2.5 rounded-full text-xs font-black tracking-wide transition-all duration-300 flex items-center gap-2 <?php echo e(request('kategori') == 'lansia' ? 'bg-orange-500 text-white shadow-md shadow-orange-500/30' : 'bg-white text-slate-500 hover:bg-orange-50 hover:text-orange-600 border border-slate-200'); ?>">
        ...
        <div class="px-8 py-6 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div class="flex flex-col">
                ...
                <?php if(request('kategori')): ?>
                    <p class="text-[11px] font-bold text-slate-400 mt-1 uppercase tracking-widest ml-[52px]">Total: <span class="text-blue-500"><?php echo e($users->total()); ?></span> Data <?php echo e(ucfirst(request('kategori'))); ?></p>
                <?php endif; ?>
            </div>
            ...
            <form method="GET" class="flex relative w-full sm:w-auto">
                ...
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari NIK / Nama..." class="w-full sm:w-80 bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm font-medium focus:bg-white focus:border-blue-400 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all shadow-sm">
            </form>
        </div>
                    ...
                    <tr class="border-b border-slate-50 hover:bg-slate-50 transition-colors text-center">
                        ...
                        <td class="py-4 px-6 text-left">
                            <div class="flex items-center gap-3 w-max">
                                ...
                                <div>
                                    <div class="font-bold text-slate-800"><?php echo e($u->profile->full_name ?? $u->name); ?></div>
                                    <div class="text-[11px] text-slate-400 flex items-center gap-1 mt-0.5">
                                        <i class="fas fa-envelope"></i> <?php echo e($u->email); ?>
                        ...
                        <td class="py-4 px-6">
                            <div class="flex items-center justify-center gap-2">
                                <a href="<?php echo e(route('admin.users.show', $u->id)); ?>" class="w-8 h-8 rounded-lg bg-sky-50 text-sky-600 hover:bg-sky-500 hover:text-white flex items-center justify-center transition-all smooth-route" title="Lihat Detail"><i class="fas fa-eye"></i></a>
                                <a href="<?php echo e(route('admin.users.edit', $u->id)); ?>" class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white flex items-center justify-center transition-all smooth-route" title="Edit Data"><i class="fas fa-edit"></i></a>
                                
                                
                                <form action="<?php echo e(route('admin.users.reset-password', $u->id)); ?>" method="POST" class="inline">
                                    <?php echo csrf_field(); ?>

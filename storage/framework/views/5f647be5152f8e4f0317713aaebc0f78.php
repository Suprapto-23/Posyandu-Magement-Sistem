...
<?php $__env->startSection('page-name', 'Kelola Jadwal Acara'); ?>
...
<div class="max-w-[1400px] mx-auto animate-slide-up pb-10">
    ...
    <div class="glass-card rounded-[24px] p-3 mb-8 flex flex-col xl:flex-row items-center justify-between gap-4 relative z-20">
        <form id="filterForm" action="<?php echo e(route('kader.jadwal.index')); ?>" method="GET" class="hidden">
            <input type="hidden" name="search" id="hiddenSearch" value="<?php echo e(request('search')); ?>">
            <input type="hidden" name="status" id="hiddenStatus" value="<?php echo e(request('status', 'semua')); ?>">
        </form>
        ...
        <div class="flex gap-2 w-full xl:w-auto overflow-x-auto custom-scrollbar pb-2 sm:pb-0 justify-start bg-slate-50/80 p-1.5 rounded-[20px] border border-slate-100">
            ...
            <?php $__currentLoopData = $tabs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <button type="button" data-status="<?php echo e($k); ?>" class="tab-btn px-5 py-2 rounded-xl font-bold text-[12px] whitespace-nowrap transition-all flex items-center gap-2 border <?php echo e($currentStatus == $k ? 'bg-white text-indigo-600 border-slate-200 shadow-sm active-tab' : 'bg-transparent text-slate-500 border-transparent hover:bg-white hover:border-slate-200'); ?>">
                   <i class="fas <?php echo e($v['icon']); ?> <?php echo e($currentStatus == $k ? 'text-indigo-500' : 'text-slate-400'); ?>"></i> <?php echo e($v['label']); ?>
        ...
        <div class="w-full xl:w-96 relative group">
            ...
            <input type="text" id="liveSearchInput" value="<?php echo e(request('search')); ?>" placeholder="Cari agenda atau lokasi..." class="search-input">
        ...
        <div id="data-wrapper">
            <?php if(isset($jadwals) && $jadwals->count() > 0): ?>
                    ...
                    <div class="jadwal-card bg-white rounded-[24px] p-6 border border-slate-100 flex flex-col shadow-sm relative group overflow-hidden">
                        ...
                        <div class="flex justify-between items-start mb-5 relative z-10">
                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 text-slate-600 text-[9px] font-black uppercase tracking-widest rounded-lg border border-slate-200 shadow-sm">
                                <i class="fas fa-users text-slate-400"></i> <?php echo e(str_replace('_', ' ', $jadwal->target_peserta)); ?>
                        ...
                        <div class="flex items-start gap-4 mb-4 relative z-10">
                            <div class="w-[60px] h-[64px] rounded-[16px] bg-indigo-50/80 border border-indigo-100 flex flex-col items-center justify-center text-indigo-600 shrink-0 group-hover:bg-indigo-600 group-hover:text-white transition-colors shadow-inner">
                                <span class="text-[10px] font-bold uppercase tracking-widest mb-0.5"><?php echo e(\Carbon\Carbon::parse($jadwal->tanggal)->translatedFormat('M')); ?></span>
                                <span class="text-[22px] font-black font-poppins leading-none"><?php echo e(\Carbon\Carbon::parse($jadwal->tanggal)->format('d')); ?></span>
                            </div>
                            
                            <div class="flex-1 min-w-0">
                                <h3 class="text-[16px] font-black text-slate-800 font-poppins leading-tight truncate mb-1 group-hover:text-indigo-700 transition-colors" title="<?php echo e($jadwal->judul); ?>"><?php echo e($jadwal->judul); ?></h3>
                                <p class="text-[11px] font-bold text-slate-500 mb-1 flex items-center gap-1.5"><i class="far fa-clock text-indigo-400 w-3 text-center"></i> <?php echo e(\Carbon\Carbon::parse($jadwal->waktu_mulai)->format('H:i')); ?> - <?php echo e(\Carbon\Carbon::parse($jadwal->waktu_selesai)->format('H:i')); ?> WIB</p>
                                <p class="text-[11px] font-bold text-slate-500 truncate flex items-center gap-1.5"><i class="fas fa-map-marker-alt text-rose-400 w-3 text-center"></i> <?php echo e($jadwal->lokasi); ?></p>
                            </div>
                        </div>
                        ...
                        ?>
                        <?php if($isPast && $jadwal->status == 'aktif'): ?>
                        ...
                        <div class="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between relative z-10">
                            <span class="text-[11px] font-bold text-slate-400 capitalize"><i class="fas fa-tag mr-1 text-slate-300"></i> <?php echo e(str_replace('_', ' ', $jadwal->kategori)); ?></span>
                            
                            <div class="flex gap-2">
                                <a href="<?php echo e(route('kader.jadwal.show', $jadwal->id)); ?>" class="loader-trigger w-9 h-9 flex items-center justify-center rounded-xl bg-slate-50 text-slate-500 hover:bg-indigo-600 hover:text-white transition-colors shadow-sm" title="Lihat Detail">
                                    <i class="fas fa-eye text-[13px]"></i>
                                </a>
                                <a href="<?php echo e(route('kader.jadwal.edit', $jadwal->id)); ?>" class="loader-trigger w-9 h-9 flex items-center justify-center rounded-xl bg-slate-50 text-slate-500 hover:bg-amber-500 hover:text-white transition-colors shadow-sm" title="Edit">
                                    <i class="fas fa-pen text-[13px]"></i>
                                </a>
                                <form action="<?php echo e(route('kader.jadwal.destroy', $jadwal->id)); ?>" method="POST" class="m-0">
                                    <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                    <button type="button" onclick="confirmDelete('<?php echo e($jadwal->id); ?>')" class="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-50 text-slate-500 hover:bg-rose-500 hover:text-white transition-colors shadow-sm" title="Hapus">
                ...
                <div id="paginationArea" class="mt-8">
                    <?php if(isset($jadwals) && $jadwals->hasPages()): ?> <?php echo e($jadwals->links()); ?> <?php endif; ?>
                </div>
...
<?php echo $__env->make('layouts.kader', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\POSYANDU\posyandu-management-system\resources\views/kader/jadwal/index.blade.php ENDPATH**/ ?>
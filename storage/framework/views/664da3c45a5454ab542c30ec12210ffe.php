...
<div class="max-w-[1400px] mx-auto animate-slide-up">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        ...
        <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
            <a href="<?php echo e(route('kader.import.index')); ?>" class="flex-1 md:flex-none justify-center flex items-center gap-2 px-5 py-3 bg-white text-emerald-600 font-black text-[13px] rounded-xl hover:bg-emerald-50 transition-all border border-emerald-200 shadow-sm hover:shadow-emerald-100 uppercase tracking-widest">
            ...
            <div class="table-container overflow-x-auto max-h-[65vh]">
                <table class="w-full text-left border-collapse whitespace-nowrap min-w-[1000px]">
                    ...
                    <tbody class="divide-y divide-slate-100" id="tableBodyBayi">
                        ...
                        ?>
                        <tr class="hover:bg-sky-50/40 transition-colors pasien-row" data-search="<?php echo e(strtolower($item->nama_lengkap . ' ' . $item->nik . ' ' . $item->nama_ibu)); ?>">
                            <td class="px-5 py-4 text-center border-r border-slate-200/50"><input type="checkbox" name="ids[]" value="<?php echo e($item->id); ?>" class="checkbox-modern row-checkbox bayi-checkbox" onchange="checkBulkStatus()"></td>
                            
                            
                            <td class="px-5 py-4 border-r border-slate-200/50">
                                <div class="flex items-center gap-3">
                                    <div class="w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg shrink-0 <?php echo e($avatarColor); ?> border border-white shadow-sm">
                                        <?php echo e(strtoupper(substr($item->nama_lengkap, 0, 1))); ?>

                                    </div>
                                    <div>
                                        <p class="text-[13px] font-black text-slate-800"><?php echo e($item->nama_lengkap); ?></p>
                                        <div class="flex items-center gap-2 mt-1">
                                            <span class="text-[9px] font-black tracking-widest px-2 py-0.5 rounded border <?php echo e($jkBadge); ?>">
                                                <?php echo e($item->jenis_kelamin == 'L' ? 'LAKI-LAKI' : 'PEREMPUAN'); ?>

                                            </span>
                                            <span class="text-[10px] font-bold text-slate-500 font-mono tracking-wide bg-slate-100 px-2 py-0.5 rounded"><i class="far fa-id-card text-slate-400"></i> <?php echo e($item->nik ?? '0000000000000000'); ?></span>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-4 border-r border-slate-200/50">
                                <p class="text-[11px] font-bold text-slate-700 mb-1.5"><i class="fas fa-map-marker-alt text-slate-400 w-3 text-center"></i> <?php echo e($item->tempat_lahir); ?>, <?php echo e(\Carbon\Carbon::parse($item->tanggal_lahir)->translatedFormat('d M Y')); ?></p>
                                <div class="flex items-center gap-2">
                                    <span class="text-[10px] font-bold text-slate-500 bg-white border border-slate-200 px-2 py-1 rounded shadow-sm">BB: <span class="text-indigo-600 font-black"><?php echo e($item->berat_lahir); ?> kg</span></span>
                                    <span class="text-[10px] font-bold text-slate-500 bg-white border border-slate-200 px-2 py-1 rounded shadow-sm">TB: <span class="text-emerald-600 font-black"><?php echo e($item->panjang_lahir); ?> cm</span></span>
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-4 border-r border-slate-200/50">
                                <div class="flex flex-col gap-1.5">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <p class="text-xs font-bold text-slate-800"><?php echo e($item->nama_ibu ?? '—'); ?></p>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <div class="w-5 h-5 rounded bg-sky-50 flex items-center justify-center text-sky-400 text-[10px]"><i class="fas fa-male"></i></div>
                                        <p class="text-[11px] font-semibold text-slate-500"><?php echo e($item->nama_ayah ?? '—'); ?></p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-4 text-center border-r border-slate-200/50">
                                <span title="<?php echo e($fullUmurTitle); ?>" class="inline-flex items-center gap-1 mb-2 px-3 py-1 rounded-lg text-[10px] font-black bg-slate-800 text-white shadow-sm cursor-help transition-transform hover:scale-105 tracking-widest uppercase">
                                    <i class="far fa-clock text-slate-300"></i> <?php echo e($strUmur); ?>
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="<?php echo e(route('kader.data.balita.show', $item->id)); ?>" class="w-9 h-9 rounded-xl bg-white border border-slate-200 text-indigo-500 hover:bg-indigo-50 hover:text-indigo-600 flex items-center justify-center transition-all shadow-sm hover:shadow-md" title="Buku KIA Digital"><i class="fas fa-book-medical"></i></a>
                                    <a href="<?php echo e(route('kader.data.balita.edit', $item->id)); ?>" class="w-9 h-9 rounded-xl bg-white border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-800 flex items-center justify-center transition-all shadow-sm hover:shadow-md" title="Edit Master Data"><i class="fas fa-edit"></i></a>
                                    
                                    <form action="<?php echo e(route('kader.data.balita.destroy', $item->id)); ?>" method="POST" id="delete-form-<?php echo e($item->id); ?>">
                                        <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                        <button type="button" onclick="confirmSingleDelete('<?php echo e($item->id); ?>', '<?php echo e(addslashes($item->nama_lengkap)); ?>')" class="w-9 h-9 rounded-xl bg-white border border-slate-200 text-rose-400 hover:bg-rose-500 hover:text-white hover:border-rose-500 flex items-center justify-center transition-all shadow-sm hover:shadow-md" title="Hapus Permanen"><i class="fas fa-trash-alt"></i></button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                ...
                <table class="w-full text-left border-collapse whitespace-nowrap min-w-[1000px]">
                    ...
                    <tbody class="divide-y divide-slate-100" id="tableBodyBalita">
                        ...
                        <?php
                            ...
                            $jkBadge = $item->jenis_kelamin == 'L' ? 'bg-sky-50 text-sky-600 border-sky-200' : 'bg-pink-50 text-pink-600 border-pink-200';
                        ?>
                        <tr class="hover:bg-rose-50/40 transition-colors pasien-row" data-search="<?php echo e(strtolower($item->nama_lengkap . ' ' . $item->nik . ' ' . $item->nama_ibu)); ?>">
                            <td class="px-5 py-4 text-center border-r border-slate-200/50"><input type="checkbox" name="ids[]" value="<?php echo e($item->id); ?>" class="checkbox-modern row-checkbox balita-checkbox" onchange="checkBulkStatus()"></td>
                            ...
                            <td class="px-5 py-4 text-center border-r border-slate-200/50">
                                <span title="<?php echo e($fullUmurTitle); ?>" class="inline-flex items-center gap-1 mb-2 px-3 py-1 rounded-lg text-[10px] font-black bg-rose-100 text-rose-700 shadow-sm cursor-help transition-transform hover:scale-105 tracking-widest uppercase">
                                    <i class="far fa-clock opacity-70"></i> <?php echo e($strUmur); ?>
...
<script>
    ...
    <?php if(session('error')): ?>
        Toast.fire({ icon: 'error', title: 'Oops...', text: "<?php echo addslashes(session('error')); ?>" });
    <?php endif; ?>

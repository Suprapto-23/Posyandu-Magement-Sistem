...
<?php $__env->startSection('page-name', 'Pemeriksaan Pasien'); ?>
...
<div class="max-w-[1400px] mx-auto animate-slide-up">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8 text-center sm:text-left">
        ...
        <a href="<?php echo e(route('kader.pemeriksaan.create')); ?>" class="inline-flex items-center justify-center gap-3 px-8 py-4 bg-indigo-600 text-white font-black text-[13px] rounded-2xl hover:bg-indigo-700 shadow-md transition-all uppercase tracking-widest w-full sm:w-auto">
    ...
    <div class="bg-white rounded-[28px] border border-slate-200/80 shadow-sm p-4 mb-8 flex flex-col lg:flex-row items-center justify-between gap-4 relative z-20">
        <div class="flex gap-2 w-full lg:w-auto overflow-x-auto custom-scrollbar pb-2 sm:pb-0 justify-start sm:justify-center">
            <?php $currentCat = request('kategori', 'semua'); ?>
            <?php $__currentLoopData = ['semua'=>'Semua', 'balita'=>'👶 Balita', 'remaja'=>'🎓 Remaja', 'ibu_hamil'=>'🤰 Ibu Hamil', 'lansia'=>'👴 Lansia']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $l): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(request()->fullUrlWithQuery(['kategori'=>$k, 'page'=>1])); ?>" 
                   class="px-5 py-2.5 rounded-xl font-extrabold text-[11px] uppercase tracking-wider whitespace-nowrap transition-all border 
                   <?php echo e($currentCat == $k ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-100 hover:text-slate-800'); ?>">
                    <?php echo e($l); ?>
        ...
        <div class="w-full lg:w-[400px] relative group">
            ...
            <input type="text" id="searchInput" value="<?php echo e($search ?? ''); ?>" placeholder="Ketik nama pasien..." class="search-input" autocomplete="off">
    ...
    <div id="table-container" class="relative">
        ...
        <div class="bg-white rounded-[32px] border border-slate-200/80 shadow-sm overflow-hidden">
            <div class="overflow-x-auto custom-scrollbar">
                <table class="w-full text-left border-collapse min-w-[900px]">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <?php $__empty_1 = true; $__currentLoopData = $pemeriksaans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-indigo-50/30 transition-colors group">
                            <td class="px-8 py-4 align-middle">
                                <p class="font-black text-slate-800 text-[13px] mb-1"><?php echo e(\Carbon\Carbon::parse($p->tanggal_periksa)->format('d M Y')); ?></p>
                                <?php if($p->kategori_pasien == 'balita'): ?> <span class="px-2 py-1 bg-violet-100 text-violet-700 text-[9px] font-black rounded uppercase border border-violet-200">Balita</span>
                                <?php elseif($p->kategori_pasien == 'remaja'): ?> <span class="px-2 py-1 bg-sky-100 text-sky-700 text-[9px] font-black rounded uppercase border border-sky-200">Remaja</span>
                                <?php elseif($p->kategori_pasien == 'ibu_hamil'): ?> <span class="px-2 py-1 bg-pink-100 text-pink-700 text-[9px] font-black rounded uppercase border border-pink-200">Ibu Hamil</span>
                                <?php else: ?> <span class="px-2 py-1 bg-emerald-100 text-emerald-700 text-[9px] font-black rounded uppercase border border-emerald-200">Lansia</span> <?php endif; ?>
                            </td>
                            <td class="px-8 py-4 align-middle">
                                <p class="font-black text-slate-800 text-[14px]"><?php echo e($p->nama_pasien); ?></p>
                                <p class="text-[10px] font-bold text-slate-500 mt-0.5 truncate max-w-[200px]"><i class="fas fa-comment-medical text-indigo-400 mr-1"></i> "<?php echo e($p->keluhan ?? 'Tidak ada keluhan'); ?>"</p>
                            </td>
                            <td class="px-8 py-4 align-middle">
                                <div class="flex items-center gap-2">
                                    <div class="px-3 py-1 bg-slate-50 border border-slate-200 rounded-lg text-center shadow-sm"><span class="text-[8px] font-black text-slate-400 uppercase mr-1">BB</span><span class="text-[12px] font-black text-slate-800"><?php echo e($p->berat_badan ?? '-'); ?></span></div>
                                    <div class="px-3 py-1 bg-slate-50 border border-slate-200 rounded-lg text-center shadow-sm"><span class="text-[8px] font-black text-slate-400 uppercase mr-1">TB</span><span class="text-[12px] font-black text-slate-800"><?php echo e($p->tinggi_badan ?? '-'); ?></span></div>
                                </div>
                            </td>
                            <td class="px-8 py-4 text-center align-middle">
                                <?php if($p->status_verifikasi == 'verified'): ?> <span class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 text-[10px] font-black border border-emerald-100 shadow-sm uppercase"><i class="fas fa-check-double text-emerald-500"></i> ACC Bidan</span>
                                <?php else: ?> <span class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-amber-50 text-amber-600 text-[10px] font-black border border-amber-100 shadow-sm uppercase"><i class="fas fa-hourglass-half text-amber-500 animate-pulse"></i> Menunggu</span> <?php endif; ?>
                            </td>
                            <td class="px-8 py-4 align-middle text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="<?php echo e(route('kader.pemeriksaan.show', $p->id)); ?>" class="w-9 h-9 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 shadow-sm transition-all" title="Detail"><i class="fas fa-folder-open"></i></a>
                                    <a href="<?php echo e(route('kader.pemeriksaan.edit', $p->id)); ?>" class="w-9 h-9 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-500 hover:text-amber-600 hover:bg-amber-50 shadow-sm transition-all" title="Edit"><i class="fas fa-pen"></i></a>
                                    <form action="<?php echo e(route('kader.pemeriksaan.destroy', $p->id)); ?>" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ukuran ini secara permanen?');" class="inline-block m-0">
            ...
            <div id="paginationArea" class="px-8 py-4 border-t border-slate-100 bg-slate-50/50"><?php if(isset($pemeriksaans) && $pemeriksaans->hasPages()): ?> <?php echo e($pemeriksaans->links()); ?> <?php endif; ?></div>
        </div>
    </div>
</div>

...
<div class="max-w-[1400px] mx-auto relative pb-16 fade-in-up">
    ...
    <div class="nexus-glass overflow-hidden flex flex-col" x-data="{ searchQuery: '<?php echo e(request('search')); ?>' }">
        ...
        <div class="px-6 md:px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-5">
            ...
            <form id="searchForm" method="GET" action="<?php echo e(route('bidan.imunisasi.index')); ?>" class="relative w-full sm:w-[400px]">
                    ...
                    <tr class="tr-nexus group" 
                        ...
                        <td class="py-5 px-6">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-[14px] <?php echo e($config['theme']); ?> flex items-center justify-center shrink-0 border shadow-sm group-hover:scale-110 transition-transform">
                                    <i class="fas <?php echo e($config['ico']); ?> text-lg"></i>
                                </div>
                                <div>
                                    <p class="font-black text-slate-800 text-[14px] mb-1 group-hover:text-cyan-600 transition-colors font-poppins"><?php echo e($nama); ?></p>
                                    <div class="flex items-center gap-2">
                                        <span class="text-[9px] font-black uppercase tracking-widest bg-white border px-2 py-0.5 rounded shadow-sm <?php echo e($config['badge']); ?>"><?php echo e($config['label']); ?></span>
                                        <span class="text-[10px] font-bold text-slate-400">ID: #<?php echo e(str_pad($imu->id, 5, '0', STR_PAD_LEFT)); ?></span>
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="py-5 px-6">
                            <div class="flex flex-col items-start gap-2">
                                <span class="text-cyan-700 bg-cyan-50 border border-cyan-100 px-3 py-1 rounded-lg font-black text-[13px] tracking-wide"><?php echo e($imu->vaksin); ?></span>
                                <div class="flex items-center gap-2">
                                    <span class="text-slate-500 bg-slate-50 border border-slate-200 px-2 py-0.5 rounded text-[10px] font-bold">Dosis: <?php echo e($imu->dosis); ?></span>
                                    
                                    <?php if($imu->keterangan && $imu->keterangan != '-'): ?>
                                        <span class="text-amber-600 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1" title="<?php echo e($imu->keterangan); ?>">
                        ...
                        <td class="py-5 px-6">
                            <p class="font-bold text-slate-700 text-[12px] flex items-center gap-2">
                                <i class="far fa-calendar-check text-cyan-500"></i> <?php echo e(\Carbon\Carbon::parse($imu->tanggal_imunisasi)->translatedFormat('d M Y')); ?>
                            ...
                            <div class="mt-1.5 inline-flex items-center gap-1.5 px-2 py-1 bg-indigo-50 border border-indigo-100 rounded text-[9px] font-black text-indigo-700 uppercase tracking-widest ml-5">
                                <i class="fas fa-user-nurse text-indigo-400"></i> <?php echo e(Str::words($imu->kunjungan->petugas->name ?? 'Sistem', 2, '')); ?>
        ...
        <div class="px-8 py-5 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest hidden sm:block">
                Menampilkan <span class="text-slate-800"><?php echo e($imunisasis->firstItem()); ?></span> - <span class="text-slate-800"><?php echo e($imunisasis->lastItem()); ?></span> dari <span class="text-slate-800"><?php echo e($imunisasis->total()); ?></span> Vaksinasi
            </p>

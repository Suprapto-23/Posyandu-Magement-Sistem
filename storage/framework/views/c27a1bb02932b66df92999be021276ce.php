...
<div class="max-w-[1400px] mx-auto pb-10">
    ...
    <div class="glass-panel rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.03)] p-6 sm:p-8 mb-10 animate-slide-up-delay-1 relative overflow-hidden">
        ...
        <form method="GET" action="<?php echo e(route('kader.absensi.riwayat')); ?>" class="flex flex-col lg:flex-row gap-5 lg:items-end relative z-10">
            ...
            <div class="flex-1 w-full">
                ...
                <select name="kategori" class="w-full border-2 border-slate-200 rounded-2xl px-5 py-4 text-[14px] font-bold text-slate-800 bg-slate-50 hover:bg-white focus:outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-50 transition-all cursor-pointer" onchange="this.form.submit()">
                    ...
                    <option value="bayi"      <?php echo e(request('kategori') == 'bayi'      ? 'selected' : ''); ?>>🍼 Bayi (0–11 Bulan)</option>
                    <option value="balita"    <?php echo e(request('kategori') == 'balita'    ? 'selected' : ''); ?>>👶 Balita (12–59 Bulan)</option>
                    <option value="remaja"    <?php echo e(request('kategori') == 'remaja'    ? 'selected' : ''); ?>>🎓 Remaja Umum</option>
                    <option value="ibu_hamil" <?php echo e(request('kategori') == 'ibu_hamil' ? 'selected' : ''); ?>>🤰 Ibu Hamil</option>
                    <option value="lansia"    <?php echo e(request('kategori') == 'lansia'    ? 'selected' : ''); ?>>👴 Lansia (Lanjut Usia)</option>
                </select>
            </div>
            ...
            <div class="w-full lg:w-auto flex gap-3">
                ...
                <?php if(request('kategori') || request('bulan')): ?>
                <a href="<?php echo e(route('kader.absensi.riwayat')); ?>" class="px-5 py-4 bg-rose-50 text-rose-600 border border-rose-200 font-black text-[13px] rounded-2xl hover:bg-rose-500 hover:text-white transition-all flex items-center justify-center shadow-sm" title="Bersihkan Filter">
            ...
            <table class="w-full text-left border-collapse min-w-[1000px]">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td class="px-6 py-5 border-r border-slate-100/50">
                            <div class="flex items-center gap-4">
                                <div class="w-14 h-14 rounded-[16px] bg-slate-50 flex flex-col items-center justify-center shrink-0 border border-slate-200 group-hover:bg-indigo-50 group-hover:border-indigo-200 group-hover:text-indigo-600 transition-all shadow-sm">
                                    ...
                                    <span class="text-xl font-black font-poppins leading-none"><?php echo e($abs->nomor_pertemuan); ?></span>
                                </div>
                                <div>
                                    <p class="font-black text-slate-800 text-[14px] group-hover:text-indigo-600 transition-colors font-poppins"><?php echo e($abs->kode_absensi); ?></p>
                                    <div class="flex items-center gap-1 mt-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                        <i class="fas fa-user-edit text-slate-300"></i> Pencatat: <?php echo e($abs->pencatat?->name ?? 'Sistem'); ?>
                        ...
                        <td class="px-6 py-5 border-r border-slate-100/50">
                            <span class="inline-flex px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border <?php echo e($badgeStyle); ?>">
                        ...
                        <td class="px-6 py-5 border-r border-slate-100/50">
                            <p class="text-[14px] font-black text-slate-800">
                                <?php echo e($abs->tanggal_posyandu->translatedFormat('d M Y')); ?>

                            </p>
                            <p class="text-[11px] text-slate-500 font-bold mt-1"><i class="far fa-clock text-slate-400 mr-1"></i> Pukul <?php echo e($abs->created_at->format('H:i')); ?> WIB</p>
                        </td>
                        ...
                        <td class="px-6 py-5 border-r border-slate-100/50">
                            <div class="w-full max-w-[200px] mx-auto">
                                <div class="flex justify-between items-end mb-2">
                                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Hadir: <span class="text-slate-800 text-[13px]"><?php echo e($hadir); ?></span>/<?php echo e($total); ?></p>
                                    <p class="text-[12px] font-black <?php echo e($pct >= 75 ? 'text-emerald-600' : ($pct >= 40 ? 'text-amber-600' : 'text-rose-600')); ?>"><?php echo e($pct); ?>%</p>
                                </div>
                                <div class="w-full h-2 bg-slate-100 border border-slate-200/50 rounded-full overflow-hidden p-[1.5px]">
                                    <div class="h-full rounded-full transition-all duration-1000 ease-out <?php echo e($progressColor); ?>" style="width:<?php echo e($pct); ?>%"></div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-5 text-center">
                            <div class="flex items-center justify-center gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                ...
                                <form action="<?php echo e(route('kader.absensi.destroy', $abs->id)); ?>" method="POST" class="inline-block" onsubmit="return confirm('PERINGATAN! Menghapus arsip ini akan menghilangkan data kehadiran secara permanen. Lanjutkan?');">

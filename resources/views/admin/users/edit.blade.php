    ...
    <div class="bg-gradient-to-br from-blue-600 to-sky-400 rounded-[2.5rem] p-8 md:p-10 mb-8 relative overflow-hidden shadow-lg border border-white/20 flex flex-col items-center justify-center text-center group">
        ...
        <div class="relative z-10 w-full flex flex-col items-center">
            ...
            <h2 class="text-3xl font-black text-white font-poppins tracking-tight flex items-center justify-center gap-3 text-shadow-sm">
                <i class="fas fa-user-edit"></i> Edit: {{ $user->profile->full_name ?? $user->name }}
            </h2>
        </div>
    </div>
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden mb-8">
            <div class="bg-slate-50/50 px-8 py-6 border-b border-slate-50 flex items-center justify-center">
                <h5 class="font-black text-slate-700 text-sm uppercase tracking-widest flex items-center gap-2">
                    <i class="fas fa-id-card-alt text-blue-500"></i> Perbarui Data Pribadi
                </h5>
            </div>
            ...
            <div class="p-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="col-span-1 md:col-span-2 space-y-2 text-center md:text-left">
                        ...
                        <input type="text" name="nik" id="nik" value="{{ old('nik', $user->profile?->nik ?? $user->nik) }}" maxlength="16" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:border-blue-400 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all">
                        <p class="text-[10px] font-medium text-slate-400 mt-1"><i class="fas fa-info-circle"></i> Jika NIK diubah, ID Login pengguna juga akan otomatis diperbarui.</p>
                    </div>
                    ...
                    <div class="space-y-2 text-center md:text-left">
                        ...
                        <input type="text" name="full_name" value="{{ old('full_name', $user->profile?->full_name ?? $user->name) }}" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:border-blue-400 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all">
                    </div>
                    ...
                    <div class="space-y-2 text-center md:text-left">
                        ...
                        <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $user->profile?->tempat_lahir) }}" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:border-blue-400 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all">
                    </div>
                    ...
                    <div class="space-y-2 text-center md:text-left">
                        ...
                        <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $user->profile?->tanggal_lahir) }}" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:border-blue-400 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all">
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-2 space-y-2 text-center md:text-left">
                        ...
                        <input type="text" name="telepon" id="telepon" value="{{ old('telepon', $user->profile?->telepon) }}" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:border-blue-400 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all">
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-2 space-y-2 text-center md:text-left">
                        ...
                        <input type="text" name="alamat" value="{{ old('alamat', $user->profile?->alamat) }}" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:border-blue-400 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all">
                    </div>

                    <div class="col-span-1 md:col-span-2 space-y-2 text-center md:text-left mt-2 border-t border-slate-100 pt-6">
                        <label class="text-[11px] font-black text-slate-500 uppercase tracking-widest">Akses Login Warga <span class="text-rose-500">*</span></label>
                        <select name="status" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-black text-slate-800 focus:bg-white focus:border-blue-400 outline-none transition-all cursor-pointer">
                            <option value="active" {{ old('status', $user->status) == 'active' ? 'selected' : '' }}>🟢 DIIZINKAN MASUK</option>
                            <option value="inactive" {{ old('status', $user->status) == 'inactive' ? 'selected' : '' }}>🔴 BLOKIR SEMENTARA</option>
                        </select>
                    </div>
                </div>
            </div>

...
<div class="max-w-6xl mx-auto space-y-8">
    ...
    {{-- Hero Section Premium (Sky & Indigo) --}}
    <div class="bg-gradient-to-br from-sky-500 to-indigo-500 rounded-[2.5rem] p-10 relative overflow-hidden shadow-[0_20px_40px_-10px_rgba(99,102,241,0.3)] border border-white/20 flex flex-col items-center justify-center text-center group animate-pop-in">
        ...
        <div class="relative z-10">
            ...
            <a href="{{ route('admin.kaders.create') }}" class="inline-flex items-center gap-2 bg-white hover:bg-slate-50 text-indigo-600 font-black px-7 py-3.5 rounded-xl transition-all shadow-lg hover:shadow-xl hover:-translate-y-1 smooth-route">
    ...
    <div class="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-center text-center gap-3 text-emerald-700 font-bold shadow-sm animate-pop-in delay-100">
        ...
        @if(session('reset_password'))
            <span class="bg-white px-3 py-1 rounded-lg border border-emerald-200 text-xs font-mono text-slate-700 shadow-sm">Pass Baru: <span class="text-indigo-600">{{ session('reset_password') }}</span></span>
        @endif
    </div>
    @endif
    ...
    <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden animate-pop-in delay-200">
        <div class="px-8 py-6 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <form method="GET" class="flex relative w-full sm:w-auto">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Nama / NIK..." class="w-full sm:w-72 bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm font-medium focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all shadow-sm">
            </form>
        </div>
                    ...
                    <tr class="border-b border-slate-50 hover:bg-slate-50 transition-colors text-center">
                        ...
                        <td class="py-4 px-6">
                            <div class="flex items-center justify-center md:justify-start gap-3 w-max mx-auto">
                                ...
                                <div class="text-left">
                                    <div class="font-bold text-slate-800">{{ $k->profile->full_name ?? $k->name }}</div>
                        ...
                        <td class="py-4 px-6">
                            <div class="flex flex-col items-center gap-1">
                                ...
                                <span class="text-[10px] font-black text-indigo-500 uppercase tracking-widest">{{ $k->kader?->jabatan ?? 'Kader Biasa' }}</span>
                            </div>
                        </td>
                        ...
                        <td class="py-4 px-6">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.kaders.show', $k->id) }}" class="w-8 h-8 rounded-lg bg-sky-50 text-sky-500 hover:bg-sky-500 hover:text-white flex items-center justify-center transition-all smooth-route" title="Detail"><i class="fas fa-eye"></i></a>
                                <a href="{{ route('admin.kaders.edit', $k->id) }}" class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-500 hover:bg-indigo-500 hover:text-white flex items-center justify-center transition-all smooth-route" title="Edit"><i class="fas fa-edit"></i></a>

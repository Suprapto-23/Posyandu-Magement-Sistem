...
<head>
    ...
    {{-- FAVICON PREMIUM BARU (Medical Heart & Cross - Sangat Elegan) --}}
...
<body class="text-slate-800 antialiased flex pb-[76px] lg:pb-0">
    ...
    {{-- LAYAR LOADING GLOBAL BARU (Smooth & Elegan) --}}
    ...
    <div class="flex-1 lg:ml-[280px] min-h-screen flex flex-col w-full relative z-10 transition-all duration-300">
        ...
        {{-- TOP APP BAR (Khusus Mobile & Tablet) --}}
        <header class="h-[72px] glass-header sticky top-0 z-40 flex items-center justify-between px-5 lg:hidden shadow-sm">
            ...
            <div class="flex items-center gap-3 relative">
                ...
                <a href="{{ route('user.notifikasi.index') }}" class="smooth-route relative w-10 h-10 flex items-center justify-center text-slate-400 hover:text-teal-600 hover:bg-teal-50 rounded-full transition-colors">
                    <i class="fas fa-bell text-[20px]"></i>
                    <span id="notifBadgeTop" class="absolute top-2 right-2 w-2.5 h-2.5 bg-rose-500 rounded-full ring-2 ring-white animate-pulse {{ $unreadCount > 0 ? '' : 'hidden' }}"></span>
                </a>
                ...
                <div class="relative">
                    ...
                    <div id="userDropdown" class="hidden absolute top-14 right-0 w-56 bg-white rounded-2xl shadow-[0_15px_40px_-10px_rgba(0,0,0,0.15)] border border-slate-100 animate-pop overflow-hidden">
                        <div class="px-5 py-4 border-b border-slate-50 bg-slate-50/50">
                            <p class="text-[13px] font-bold text-slate-800 truncate">{{ Auth::user()->name ?? 'Warga' }}</p>
                        </div>
                        <div class="p-2">
                            <a href="{{ route('user.profile.edit') }}" class="smooth-route w-full flex items-center gap-3 px-4 py-3 text-[12px] font-bold text-slate-500 hover:bg-teal-50 hover:text-teal-600 rounded-xl transition-all"><i class="fas fa-user-circle w-4 text-center text-lg"></i> Profil Saya</a>
        ...
        {{-- KONTEN HALAMAN (Dirender dari View Lain) --}}
        <main class="flex-1 p-4 sm:p-6 lg:p-8 w-full max-w-[1400px] mx-auto relative z-0">
            
            {{-- Alert Dinamis --}}
            @foreach (['success' => ['bg-emerald-50', 'text-emerald-600', 'fa-check-circle', 'border-emerald-200'], 'error' => ['bg-rose-50', 'text-rose-600', 'fa-exclamation-triangle', 'border-rose-200']] as $msg => $cls)
                @if(session($msg))
                    <div class="mb-6 px-5 py-4 {{ $cls[0] }} border {{ $cls[3] }} rounded-[16px] flex items-center justify-between shadow-sm animate-pop">
                        <div class="flex items-center gap-3">
                            <i class="fas {{ $cls[2] }} {{ $cls[1] }} text-xl shrink-0"></i>
                            <span class="{{ $cls[1] }} text-[13px] font-bold leading-tight">{{ session($msg) }}</span>
                        </div>
                        <button onclick="this.parentElement.style.display='none'" class="{{ $cls[1] }} opacity-50 hover:opacity-100 p-2 transition-opacity"><i class="fas fa-times"></i></button>
                    </div>
                @endif
            @endforeach
    ...
    {{-- BOTTOM NAVIGATION (Khusus Mobile, Gaya Aplikasi Modern) --}}
    <nav class="lg:hidden fixed bottom-0 left-0 w-full glass-bottom-nav z-50 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.04)]">
        <div class="flex justify-between items-end px-2 relative h-[65px]">
            ...
            <a href="{{ route('user.dashboard') }}" class="smooth-route nav-item {{ request()->routeIs('user.dashboard') ? 'active' : '' }}">
                <i class="fas fa-home text-[20px] mb-1 transition-transform"></i>
                <span class="text-[10px] tracking-wide">Beranda</span>
            </a>
            
            <a href="{{ route('user.jadwal.index') }}" class="smooth-route nav-item {{ request()->routeIs('user.jadwal.*') ? 'active' : '' }}">
                <i class="fas fa-calendar-alt text-[20px] mb-1 transition-transform"></i>
                <span class="text-[10px] tracking-wide">Jadwal</span>
            </a>
            
            {{-- Tombol Tengah Mengambang (Pusat Rekam Medis) --}}
            <div class="w-full flex justify-center relative">
                <a href="{{ route('user.riwayat.index') }}" class="smooth-route center-fab">
                    <i class="fas fa-notes-medical"></i>
                </a>
                <span class="absolute bottom-3 text-[10px] font-bold text-teal-600 tracking-wide mt-2">KMS Medis</span>
            </div>
            
            <a href="{{ route('user.notifikasi.index') }}" class="smooth-route nav-item {{ request()->routeIs('user.notifikasi.*') ? 'active' : '' }}">
                <div class="relative">
            ...
            <a href="{{ route('user.profile.edit') }}" class="smooth-route nav-item {{ request()->routeIs('user.profile.*') ? 'active' : '' }}">
    ...
    {{-- SCRIPTS INTI APLIKASI (Termasuk AJAX Polling Real-time) --}}
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            let currentUnread = {{ \App\Models\Notifikasi::where('user_id', Auth::id())->where('is_read', false)->count() }};

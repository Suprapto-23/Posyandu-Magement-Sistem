...
<body class="flex h-screen overflow-hidden selection:bg-indigo-500 selection:text-white" 
        ...
        <header class="h-[75px] sm:h-[85px] nexus-glass-header sticky top-0 z-30 flex items-center justify-between px-4 sm:px-6 lg:px-10">
            ...
            <div class="flex items-center gap-4 sm:gap-5">
                ...
                <div class="hidden md:flex flex-col">
                    <h2 class="text-[22px] font-black text-slate-800 tracking-tight font-poppins leading-none">@yield('page-name', 'Beranda Utama')</h2>
            ...
            <div class="flex items-center gap-3 sm:gap-5 relative">
                ...
                @php $unreadNotifCount = class_exists('\App\Models\Notifikasi') ? \App\Models\Notifikasi::where('user_id', Auth::id() ?? 1)->where('is_read', false)->count() : 0; @endphp
                ...
                <div class="relative">
                    <button @click="notifOpen = !notifOpen; profileOpen = false" @click.away="notifOpen = false" class="relative w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center bg-white text-slate-500 hover:text-indigo-600 rounded-[14px] sm:rounded-[16px] transition-all border border-slate-200/60 shadow-sm hover:shadow-lg hover:-translate-y-0.5 group">
                        ...
                        <span id="notifBadge" class="absolute -top-1 -right-1 flex h-3.5 w-3.5 sm:h-4 sm:w-4 {{ $unreadNotifCount > 0 ? '' : 'hidden' }}">
                    ...
                    {{-- DROPDOWN NOTIFIKASI (SMART RESPONSIVE) --}}
                    <div x-show="notifOpen" 
                        ...
                        <div class="px-5 sm:px-6 py-4 sm:py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            ...
                            <span id="notifCount" class="text-[9px] sm:text-[10px] font-black px-3 py-1 rounded-full text-rose-600 bg-rose-50 border border-rose-100 {{ $unreadNotifCount > 0 ? '' : 'hidden' }} shadow-sm">{{ $unreadNotifCount }} Baru</span>
                        </div>
                        ...
                        <div class="p-3 sm:p-4 border-t border-slate-100 bg-slate-50/80">
                            <a href="{{ route('kader.notifikasi.index') }}" class="spa-route w-full py-3 sm:py-3.5 text-[10px] sm:text-[11px] font-black tracking-widest uppercase text-indigo-600 hover:text-white hover:bg-indigo-600 rounded-xl sm:rounded-2xl transition-all block text-center shadow-sm border border-indigo-100">
                                Lihat Seluruh Arsip
                            </a>
                        </div>
                    </div>
                ...
                <div class="relative">
                    <button @click="profileOpen = !profileOpen; notifOpen = false" @click.away="profileOpen = false" class="flex items-center gap-3 pl-1.5 pr-1.5 sm:pr-4 py-1.5 bg-white border border-slate-200/60 rounded-[14px] sm:rounded-[18px] shadow-sm hover:shadow-lg transition-all hover:-translate-y-0.5 group">
                        ...
                        <div class="hidden sm:block text-left">
                            <p class="text-[13px] font-black text-slate-800 leading-none truncate max-w-[130px] font-poppins">{{ Auth::user()->name ?? 'Kader Aktif' }}</p>
                    ...
                    {{-- DROPDOWN PROFIL (SMART RESPONSIVE) --}}
                    <div x-show="profileOpen" 
                        ...
                        <div class="px-4 py-3 sm:px-5 sm:py-4 border-b border-slate-100 mb-2 bg-slate-50/80 rounded-t-[20px]">
                            ...
                            <p class="text-[12px] sm:text-[13px] font-bold text-slate-800 truncate mt-1">{{ Auth::user()->email ?? 'kader@posyandu.com' }}</p>
                        </div>
                        
                        <a href="{{ route('kader.profile.index') }}" class="spa-route flex items-center gap-3 sm:gap-4 px-4 py-3 sm:px-5 sm:py-3.5 text-[11px] sm:text-[12px] font-black uppercase tracking-wider text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-colors">
    ...
    {{-- =========================================================
         SYSTEM SCRIPTS & AJAX POLLING ENGINE
         ========================================================= --}}
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            let currentUnreadCount = {{ $unreadNotifCount ?? 0 }};

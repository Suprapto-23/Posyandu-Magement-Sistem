        ...
        <div class="relative z-10 flex flex-col pt-6 pb-2 shrink-0">
            ...
            <div class="px-5">
                <div class="p-3 bg-white/80 backdrop-blur border border-slate-100 rounded-2xl flex items-center gap-3 shadow-[0_2px_10px_rgba(0,0,0,0.02)] cursor-pointer group" onclick="document.getElementById('userDropdownBtn').click()">
                    <div class="w-11 h-11 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center font-black text-lg shrink-0 group-hover:bg-indigo-600 group-hover:text-white transition-colors">{{ strtoupper(substr(Auth::user()->profile->full_name ?? Auth::user()->name ?? 'K', 0, 1)) }}</div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-bold text-slate-900 truncate">{{ Auth::user()->profile->full_name ?? Auth::user()->name ?? 'Kader' }}</p>
    ...
    <div class="flex-1 lg:ml-[280px] min-h-screen flex flex-col w-full relative">
        <header class="h-20 glass-header border-b border-slate-200/80 sticky top-0 z-40 flex items-center justify-between px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center gap-2 sm:gap-4 relative">
                        ...
                        <div id="notifList" class="max-h-72 overflow-y-auto custom-scrollbar flex-1">
                            @forelse($latestNotifs as $n)
                                <a href="{{ route('kader.notifikasi.index') }}" class="notif-item {{ $n->is_read ? '' : 'unread' }} flex gap-4 px-5 py-4 hover:bg-slate-50 transition-colors border-b border-slate-100 {{ $n->is_read ? 'bg-white border-l-4 border-l-transparent' : 'bg-indigo-50/40 border-l-4 border-l-indigo-500' }}">
                                    <div class="w-9 h-9 rounded-full flex items-center justify-center shrink-0 border {{ $n->is_read ? 'bg-slate-100 text-slate-500 border-transparent' : 'bg-indigo-100 text-indigo-600 border-indigo-200' }}">
                                    ...
                                    <div>
                                        <p class="text-[13px] font-bold {{ $n->is_read ? 'text-slate-600' : 'text-slate-800' }} leading-tight">{{ $n->judul }}</p>
                ...
                <div class="static sm:relative ml-1">
                    <button id="userDropdownBtn" class="w-10 h-10 rounded-full border border-slate-200 overflow-hidden flex items-center justify-center bg-white text-indigo-600 font-bold hover:ring-2 hover:ring-indigo-100 transition-all shadow-sm">{{ strtoupper(substr(Auth::user()->profile->full_name ?? Auth::user()->name ?? 'K', 0, 1)) }}</button>
                    <div id="userDropdown" class="hidden absolute top-16 right-0 w-[calc(100vw-2rem)] sm:top-auto sm:right-0 sm:mt-3 sm:w-60 bg-white rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] border border-slate-200 z-50 animate-pop overflow-hidden">
                        <div class="px-5 py-4 border-b border-slate-100 bg-slate-50/50">
                            <p class="text-[14px] font-bold text-slate-900 truncate">{{ Auth::user()->profile->full_name ?? Auth::user()->name ?? 'Kader' }}</p>
                            <p class="text-[12px] text-slate-500 mt-0.5 truncate">{{ Auth::user()->email ?? 'kader@posyandu.com' }}</p>
                        </div>
                        <div class="p-2">
                            <a href="{{ route('kader.profile.index') }}" class="w-full flex items-center gap-3 px-4 py-2.5 text-[13px] font-medium text-slate-600 hover:bg-slate-50 hover:text-indigo-600 rounded-xl transition-all"><i class="fas fa-user-circle w-4 text-center"></i> Profil Saya</a>
        ...
        <main class="flex-1 p-4 sm:p-6 lg:p-8 max-w-[1400px] mx-auto w-full relative z-0">
            @foreach (['success' => ['bg-emerald-50', 'text-emerald-800', 'text-emerald-500', 'fa-check-circle', 'border-emerald-200'], 'error' => ['bg-rose-50', 'text-rose-800', 'text-rose-500', 'fa-circle-exclamation', 'border-rose-200']] as $msg => $classes)
                @if(session($msg))
                    <div class="mb-6 px-5 py-4 {{ $classes[0] }} border {{ $classes[4] }} rounded-xl flex items-center justify-between shadow-sm animate-[slideDown_0.4s_ease-out]">
                        <div class="flex items-center gap-3"><i class="fas {{ $classes[3] }} {{ $classes[2] }} text-xl"></i><span class="{{ $classes[1] }} text-[14px] font-semibold">{{ session($msg) }}</span></div>

...
<div class="space-y-6">
   ...
   {{-- =========================================================================
    ...
    {{-- =========================================================================
         2. INTERACTIVE STAT CARDS (SEKARANG BISA DIKLIK / HYPERLINK)
         ========================================================================= --}}
    @php
    ...
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-4 fade-in-scale delay-100">
        @foreach($cards as $c)
        {{-- FIX: Dibungkus dengan tag <a> agar berfungsi sebagai tombol navigasi --}}
        <a href="{{ route($c['route']) }}" class="premium-card stat-card p-5 flex flex-col relative group cursor-pointer" style="--card-color: {{ $c['color'] }};">
            <div class="stat-bg-blob"></div>
            <div class="relative z-10 flex flex-col items-start w-full">
                <div class="flex justify-between items-start w-full">
                    <div class="icon-wrapper" style="background:{{ $c['bg'] }}; color:{{ $c['color'] }}; box-shadow: 0 4px 10px {{ $c['shadow'] }};">
                        <i class="fas {{ $c['icon'] }}"></i>
                    </div>
                    {{-- Indikator panah muncul saat di-hover --}}
                    <div class="w-6 h-6 rounded-full bg-slate-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" style="color:{{ $c['color'] }};">
                        <i class="fas fa-arrow-right text-[10px]"></i>
                    </div>
                </div>
                <p class="text-[28px] font-black font-poppins text-slate-800 leading-none mb-1 group-hover:scale-105 transition-transform transform origin-left">{{ $c['val'] }}</p>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest group-hover:text-slate-600 transition-colors">{{ $c['label'] }}</p>
            </div>
        </a>
        @endforeach
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-5 fade-in-up delay-200">
        ...
        <div class="xl:col-span-2 premium-card p-6 flex flex-col">
            <div class="flex items-center justify-between mb-6">
                ...
                {{-- Filter Dibuat Fungsional-Semu untuk RAD (Bisa dimatikan atau dikembangkan) --}}
        ...
        <div class="premium-card p-6 flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto pr-2 custom-scrollbar" style="max-height:280px;">
                <div class="timeline-container">
                    @forelse($kunjungan_baru ?? [] as $kunj)
                        ...
                        @endphp
                        <div class="tl-item">
                            <div class="tl-dot {{ $borderColor }} flex items-center justify-center">
                            ...
                            <div class="bg-slate-50 border border-slate-100 rounded-2xl p-3 ml-2 hover:bg-white hover:border-indigo-200 transition-all">
                                <div class="flex items-start justify-between gap-2 mb-1">
                                    <p class="text-[12px] font-bold text-slate-800 leading-tight truncate">{{ $kunj->pasien->nama_lengkap ?? 'Warga' }}</p>
                                    <span class="text-[9px] text-slate-400 font-bold bg-white px-1.5 py-0.5 rounded border border-slate-100 whitespace-nowrap">{{ $kunj->created_at->diffForHumans(null, true, true) }}</span>
                                </div>
            ...
            <div class="mt-4 pt-3 border-t border-slate-100">
                <a href="{{ route('kader.kunjungan.index') }}" class="block w-full text-center text-[11px] font-black font-poppins text-indigo-600 hover:text-indigo-800 uppercase tracking-widest bg-indigo-50/50 hover:bg-indigo-50 py-2 rounded-xl transition-colors">Tampilkan Semua</a>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-5 fade-in-up delay-300">
        ...
        <div class="xl:col-span-2 premium-card p-6 bg-gradient-to-br from-white to-slate-50/50">
            <div class="flex items-center justify-between mb-6 border-b border-slate-100 pb-3">
                ...
                <a href="{{ route('kader.jadwal.index') }}" class="text-[10px] font-black font-poppins text-slate-600 hover:text-indigo-600 bg-white border border-slate-200 px-3 py-1.5 rounded-lg transition-all uppercase tracking-wider">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <a href="{{ route('kader.jadwal.show', $jdw->id) }}" class="jadwal-list-item group">
                    <div class="jadwal-cal">
                        <span class="text-[9px] font-black font-poppins text-slate-400 uppercase tracking-widest group-hover:text-indigo-500 transition-colors">{{ \Carbon\Carbon::parse($jdw->tanggal)->translatedFormat('M') }}</span>
                        <span class="text-[20px] font-black font-poppins text-slate-800 leading-none mt-0.5 group-hover:text-indigo-600">{{ \Carbon\Carbon::parse($jdw->tanggal)->format('d') }}</span>
                    </div>
                    <div class="flex-1 min-w-0 border-l border-slate-200/60 pl-3">
                        <h4 class="text-[13px] font-bold font-poppins text-slate-800 truncate mb-1 group-hover:text-indigo-700 transition-colors">{{ $jdw->judul }}</h4>
                        <div class="flex items-center gap-2">
                            <span class="inline-flex items-center gap-1 text-[10px] font-bold text-slate-500 bg-white px-1.5 py-0.5 rounded border border-slate-100">
                                <i class="far fa-clock text-indigo-400"></i> {{ \Carbon\Carbon::parse($jdw->waktu_mulai)->format('H:i') }}
                            </span>
                            <span class="inline-flex items-center gap-1 text-[10px] font-bold text-slate-500 truncate">
                                <i class="fas fa-map-marker-alt text-rose-400"></i> {{ Str::limit($jdw->lokasi, 15) }}
                            </span>
                        </div>
                    </div>
            ...
            <div class="relative mx-auto mb-6 mt-2" style="width:180px;height:180px;">
                @if($totalW > 0)
                    ...
                    <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        ...
                        <span class="text-[28px] font-black font-poppins text-slate-800 leading-none">{{ $totalW }}</span>
                    </div>
                @else
            ...
            <div class="grid grid-cols-2 gap-2 mt-auto">
                ...
                <div class="flex items-center justify-between p-2.5 rounded-xl border border-slate-100 bg-slate-50 hover:bg-white hover:shadow-sm transition-all group">
                    <div class="flex items-center gap-1.5">
                        <span class="w-2.5 h-2.5 rounded-full {{ $d['color'] }} shadow-sm"></span>
                        <span class="text-[10px] font-bold text-slate-600 uppercase tracking-wider font-poppins">{{ $d['label'] }}</span>
                    </div>
                    <span class="text-[11px] font-black font-poppins text-slate-800 bg-white px-1.5 py-0.5 rounded border {{ $d['border'] }} shadow-sm">{{ $d['val'] }}</span>
                </div>
                @endforeach
            </div>
        </div>
...
@push('scripts')
{{-- MEMANGGIL DEPENDENSI CHART.JS AGAR GRAFIK BERFUNGSI --}}

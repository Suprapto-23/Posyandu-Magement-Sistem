...
<div class="max-w-[1400px] mx-auto animate-slide-up">
    ...
    <div class="bg-white rounded-[24px] p-6 md:p-8 mb-6 border border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col md:flex-row items-start md:items-center justify-between gap-6 print:hidden">
        <div class="flex items-center gap-5">
            {{-- Ikon Bulat Estetik (Tema Emerald) --}}
            <div class="w-16 h-16 rounded-2xl bg-emerald-50 text-emerald-500 flex items-center justify-center text-3xl shrink-0">
                <i class="fas fa-wheelchair"></i>
            </div>
            <div>
                <h2 class="text-2xl md:text-3xl font-black text-slate-900 tracking-tight mb-1">Laporan Lansia</h2>
                <p class="text-slate-500 text-sm font-medium">Rekapitulasi skrining lansia (PTM & Lab Sederhana).</p>
            </div>
        </div>

        <div class="w-full md:w-auto">
            <a href="{{ route('kader.laporan.generate', ['type' => 'lansia', 'bulan' => $bulan, 'tahun' => $tahun]) }}" class="w-full md:w-auto inline-flex items-center justify-center gap-2 px-8 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm rounded-xl transition-all shadow-[0_4px_15px_rgba(16,185,129,0.3)] hover:-translate-y-1">
    ...
    <div class="bg-white rounded-[20px] border border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-4 mb-6 print:hidden">
        <form action="{{ route('kader.laporan.lansia') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-4">
            
            {{-- Input Bulan --}}
            <div class="flex-1 w-full relative flex items-center bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus-within:border-emerald-400 focus-within:bg-white transition-colors">
    ...
    <div class="bg-slate-100 rounded-[32px] border border-slate-200 p-6 md:p-10 shadow-inner overflow-x-auto print:bg-white print:p-0 print:border-none print:shadow-none">
        ...
        <div class="paper-preview bg-white mx-auto shadow-[0_15px_50px_rgba(0,0,0,0.08)] border border-slate-200 print:shadow-none print:border-none relative" style="min-width: 1000px; max-width: 1122px; padding: 60px; border-radius: 8px;">
            {{-- Mengambil komponen tabel lansia --}}

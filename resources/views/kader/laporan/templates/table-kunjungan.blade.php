...
<body class="cetak-body">
    ...
    <div class="judul-laporan">Buku Log Kunjungan & Absensi Warga</div>
    <div class="periode">Periode: {{ $namaBulan }} {{ $tahun }}</div>

    <table class="data-table">
        <thead>
            <tr>
                <th width="5%">No</th><th width="15%">Tgl Kedatangan</th><th width="20%">Nama Warga</th><th width="15%">Kategori</th><th width="25%">Layanan / Keluhan</th><th width="20%">Resepsionis</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td class="txt-center">{{ \Carbon\Carbon::parse($row->tanggal_kunjungan)->format('d/m/Y') }}</td>
                <td>{{ $row->pasien->nama_lengkap ?? 'Unknown' }}</td>
                <td class="txt-center">{{ str_replace('App\Models\\', '', $row->pasien_type) }}</td>
                <td style="font-style: italic;">{{ $row->keluhan ?? 'Kunjungan Rutin' }}</td>
                <td class="txt-center">{{ $row->petugas->name ?? 'Kader Posyandu' }}</td>
            </tr>
            @empty
            <tr><td colspan="6" class="txt-center">Tidak ada data kehadiran di bulan ini.</td></tr>
            @endforelse
        </tbody>
    </table>
    <div class="signature-wrap"><div class="signature-box">Bantar Kulon, {{ now()->translatedFormat('d F Y') }}<br>Mengetahui,<br>Bidan / Kader Posyandu<br><br><br><br><span style="font-weight: bold; text-decoration: underline;">_________________________</span><br>NIP. ..............................</div></div>
</body>
</html>
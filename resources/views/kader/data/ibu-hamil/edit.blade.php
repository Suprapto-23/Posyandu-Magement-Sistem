...
@section('page-name', 'Koreksi Kehamilan')
...
<div class="max-w-6xl mx-auto animate-slide-up relative z-10 pb-12">
    ...
    {{-- AURA BACKGROUND (Konsisten Pink) --}}
    ...
    <div class="mb-6 flex items-center gap-3 relative z-10">
        <a href="{{ route('kader.data.ibu-hamil.index') }}" onclick="showLoader()" class="w-12 h-12 rounded-[16px] bg-white border border-slate-200 text-slate-500 flex items-center justify-center hover:bg-pink-50 hover:border-pink-300 hover:text-pink-600 transition-all shadow-sm group">
    ...
    {{-- FORM UTAMA --}}
    <form action="{{ route('kader.data.ibu-hamil.update', $ibuHamil->id) }}" method="POST" id="formEditIbu" class="relative z-10">
        @csrf @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-7 glass-panel rounded-[32px] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.04)] p-8 md:p-10 relative overflow-hidden flex flex-col">
                ...
                <div class="space-y-6 flex-1">
                    <div>
                        ...
                        <input type="number" name="nik" value="{{ old('nik', $ibuHamil->nik) }}" required placeholder="16 Digit NIK KTP" class="form-input @error('nik') form-error @enderror">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $ibuHamil->nama_lengkap) }}" required placeholder="Contoh: Siti Aisyah" class="form-input @error('nama_lengkap') form-error @enderror">
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            <label class="form-label">Tempat Lahir <span class="text-rose-500">*</span></label>
                            <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $ibuHamil->tempat_lahir) }}" required placeholder="Kota Kelahiran" class="form-input">
                        </div>
                        <div>
                            <label class="form-label">Tanggal Lahir <span class="text-rose-500">*</span></label>
                            <input type="date" id="tanggal_lahir" name="tanggal_lahir" value="{{ old('tanggal_lahir', $ibuHamil->tanggal_lahir?->format('Y-m-d')) }}" required class="form-input cursor-pointer">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="nama_suami" value="{{ old('nama_suami', $ibuHamil->nama_suami) }}" required placeholder="Nama Lengkap Suami" class="form-input">
                        </div>
                        <div>
                            <label class="form-label">No. HP Keluarga (Opsional)</label>
                            <input type="number" name="telepon_ortu" value="{{ old('telepon_ortu', $ibuHamil->telepon_ortu) }}" placeholder="Contoh: 0812xxxx" class="form-input">
                        </div>
                    </div>

                    <div>
                        <label class="form-label">Alamat Domisili <span class="text-rose-500">*</span></label>
                        <textarea name="alamat" rows="2" required placeholder="Alamat lengkap RT/RW..." class="form-input resize-none">{{ old('alamat', $ibuHamil->alamat) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-5 flex flex-col gap-8">
                ...
                <div class="bg-rose-50/80 rounded-[32px] border border-rose-200/80 shadow-[0_10px_40px_-10px_rgba(236,72,153,0.03)] p-8 md:p-10 relative overflow-hidden">
                    ...
                    <div class="flex items-center justify-between mb-8 border-b border-rose-200 pb-5 relative z-10">
                        ...
                        <select name="status" class="bg-white border-2 border-rose-200 text-rose-600 font-bold text-xs rounded-full px-3 py-1.5 outline-none cursor-pointer focus:border-rose-400 shadow-sm">
                            ...
                            <option value="selesai" {{ old('status', $ibuHamil->status) == 'selesai' ? 'selected' : '' }}>Selesai (Lahir)</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-6 relative z-10">
                        <div class="bg-white p-5 rounded-[20px] border border-rose-100 shadow-sm">
                            ...
                            <input type="date" name="hpht" id="hpht" value="{{ old('hpht', $ibuHamil->hpht?->format('Y-m-d')) }}" required class="form-input bg-slate-50 border-slate-200 focus:bg-white focus:border-rose-400 mb-4 cursor-pointer">
                            
                            <label class="form-label text-rose-600"><i class="fas fa-baby mr-1"></i> HPL (Perkiraan Lahir) <span class="text-rose-500">*</span></label>
                            <input type="date" name="hpl" id="hpl" value="{{ old('hpl', $ibuHamil->hpl?->format('Y-m-d')) }}" required class="form-input bg-rose-50 border-rose-200 text-rose-800 font-black focus:bg-white focus:border-rose-400" readonly title="Sistem menghitung otomatis dari HPHT">
                ...
                {{-- KARTU FISIK DASAR (DARK THEME - Identik Create) --}}
                <div class="bg-slate-900 rounded-[32px] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.15)] p-8 md:p-10 relative overflow-hidden flex-1 flex flex-col justify-center">
                    ...
                    <div class="grid grid-cols-2 gap-5 mb-5 relative z-10">
                        <div>
                            ...
                            <input type="number" step="0.1" name="berat_badan" id="berat_badan" value="{{ old('berat_badan', $ibuHamil->berat_badan) }}" placeholder="0.0" class="form-input bg-slate-800 border-slate-700 text-white placeholder:text-slate-600 focus:bg-slate-700 focus:border-pink-500">
                        </div>
                        <div>
                            <label class="form-label text-slate-400">Tinggi (cm)</label>
                            <input type="number" step="0.1" name="tinggi_badan" id="tinggi_badan" value="{{ old('tinggi_badan', $ibuHamil->tinggi_badan) }}" placeholder="0.0" class="form-input bg-slate-800 border-slate-700 text-white placeholder:text-slate-600 focus:bg-slate-700 focus:border-pink-500">
                        </div>
                    </div>
        ...
        <div class="mt-8 bg-white border border-slate-200 p-6 md:p-8 rounded-[32px] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] flex flex-col sm:flex-row items-center justify-between gap-6 relative z-30">
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto shrink-0">
                <a href="{{ route('kader.data.ibu-hamil.index') }}" onclick="showLoader()" class="flex-1 sm:flex-none px-8 py-4 bg-slate-100 border border-slate-200 text-slate-600 font-extrabold text-[12px] rounded-full hover:bg-slate-200 transition-colors text-center uppercase tracking-widest">
                    Batalkan
                </a>

...
<div class="max-w-[1400px] mx-auto animate-slide-up">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-8">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('kader.import.index') }}" class="px-5 py-2.5 bg-emerald-50 text-emerald-600 font-extrabold text-sm rounded-xl hover:bg-emerald-100 transition-colors border border-emerald-200">
                <i class="fas fa-file-excel mr-1.5"></i> Import Excel
            </a>
            <a href="{{ route('kader.data.balita.create') }}" class="px-5 py-2.5 bg-indigo-600 text-white font-extrabold text-sm rounded-xl hover:bg-indigo-700 shadow-[0_4px_12px_rgba(79,70,229,0.3)] hover:-translate-y-0.5 transition-all">
                <i class="fas fa-plus mr-1.5"></i> Tambah Data
            </a>
        </div>
    </div>

    {{-- STATS, SWITCHER, SEARCH & BULK ACTION --}}
    <div class="glass-card rounded-3xl p-4 mb-6 flex flex-col xl:flex-row items-center gap-4 justify-between">
        ...
        <div class="flex items-center gap-3 w-full xl:w-auto">
            ...
            <form action="{{ route('kader.data.balita.bulk-delete') }}" method="POST" id="bulkDeleteForm" class="hidden">
                @csrf
        ...
        <form action="{{ route('kader.data.balita.index') }}" method="GET" class="w-full xl:w-1/3 relative group">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Ketik Nama atau NIK untuk mencari..." 
                ...
                <table class="w-full text-left border-collapse whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-indigo-50/30 transition-colors">
                            <td class="px-4 py-4 text-center"><input type="checkbox" name="ids[]" value="{{ $item->id }}" class="checkbox-modern row-checkbox bayi-checkbox" onchange="checkBulkStatus()"></td>
                            ...
                            {{-- Kelahiran & Fisik --}}
                            <td class="px-4 py-4">
                                <p class="text-xs font-bold text-slate-700 mb-1"><i class="fas fa-map-marker-alt text-slate-400 mr-1"></i> {{ $item->tempat_lahir }}, {{ \Carbon\Carbon::parse($item->tanggal_lahir)->translatedFormat('d M Y') }}</p>
                                <div class="flex items-center gap-3 text-[11px] font-bold text-slate-500">
                                    <span class="bg-slate-100 px-2 py-0.5 rounded border border-slate-200">BB: <span class="text-indigo-600">{{ $item->berat_lahir }} kg</span></span>
                                    <span class="bg-slate-100 px-2 py-0.5 rounded border border-slate-200">PB: <span class="text-emerald-600">{{ $item->panjang_lahir }} cm</span></span>
                                </div>
                            </td>

                            {{-- Orang Tua --}}
                            <td class="px-4 py-4">
                                <p class="text-xs font-bold text-slate-800 mb-0.5"><i class="fas fa-female text-rose-400 w-4"></i> {{ $item->nama_ibu ?? '-' }}</p>
                                <p class="text-[11px] font-semibold text-slate-500"><i class="fas fa-male text-blue-400 w-4"></i> {{ $item->nama_ayah ?? '-' }}</p>
                            </td>

                            {{-- Usia & Akun --}}
                            <td class="px-4 py-4 text-center">
                                <span title="{{ $fullUmurTitle }}" class="block cursor-help mb-2 w-max mx-auto px-3 py-1 rounded-lg text-[11px] font-black bg-indigo-100 text-indigo-700 border border-indigo-200 shadow-sm transition-transform hover:scale-105">
                                    <i class="fas fa-clock mr-1 opacity-70"></i> {{ strtoupper($strUmurBayi) }}
                                </span>
                                @if($item->user_id)
                                ...
                                @else
                                    <a href="{{ route('kader.data.balita.sync', $item->id) }}" class="text-[10px] font-black text-amber-600 bg-amber-50 px-2 py-1 rounded-md border border-amber-200 hover:bg-amber-100 transition-colors"><i class="fas fa-sync"></i> Tarik Akun</a>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-4 pr-6">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('kader.data.balita.show', $item->id) }}" class="w-9 h-9 rounded-xl bg-white border border-slate-200 text-indigo-500 hover:bg-indigo-50 hover:border-indigo-300 flex items-center justify-center transition-all shadow-sm" title="Lihat Rekam Medis"><i class="fas fa-file-medical"></i></a>
                                    <a href="{{ route('kader.data.balita.edit', $item->id) }}" class="w-9 h-9 rounded-xl bg-white border border-slate-200 text-amber-500 hover:bg-amber-50 hover:border-amber-300 flex items-center justify-center transition-all shadow-sm" title="Edit Data"><i class="fas fa-pen"></i></a>
                ...
                <table class="w-full text-left border-collapse whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-rose-50/30 transition-colors">
                            <td class="px-4 py-4 text-center"><input type="checkbox" name="ids[]" value="{{ $item->id }}" class="checkbox-modern row-checkbox balita-checkbox" onchange="checkBulkStatus()"></td>
                            ...
                            {{-- Usia & Akun --}}
                            <td class="px-4 py-4 text-center">
                                <span title="{{ $fullUmurTitle }}" class="block cursor-help mb-2 w-max mx-auto px-3 py-1 rounded-lg text-[11px] font-black bg-rose-100 text-rose-700 border border-rose-200 shadow-sm transition-transform hover:scale-105">
                                    <i class="fas fa-clock mr-1 opacity-70"></i> {{ strtoupper($strUmurBalita) }}
                                </span>
                                @if($item->user_id)
                                    <span class="text-[10px] font-black text-emerald-500 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-100"><i class="fas fa-check-circle"></i> Akun Tertaut</span>
                                @else

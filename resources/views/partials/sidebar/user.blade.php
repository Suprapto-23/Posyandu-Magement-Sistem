    ...
    <nav class="flex-1 overflow-y-auto px-4 py-5 space-y-7 custom-scrollbar">
        ...
        <div>
            ...
            <div class="space-y-1.5">
                <a href="{{ route('user.dashboard') }}" class="smooth-route group flex items-center gap-3.5 px-3 py-3.5 rounded-xl {{ user_nav_active('user.dashboard') }}">
                    <i class="fas fa-home w-6 text-center text-[1.15rem] {{ user_nav_icon('user.dashboard') }}"></i>
                    <span class="text-sm tracking-wide">Beranda Saya</span>
                </a>
                <a href="{{ route('user.jadwal.index') }}" class="smooth-route group flex items-center gap-3.5 px-3 py-3.5 rounded-xl {{ user_nav_active('user.jadwal.*') }}">
                    <i class="fas fa-calendar-check w-6 text-center text-[1.15rem] {{ user_nav_icon('user.jadwal.*') }}"></i>
                    <span class="text-sm tracking-wide">Jadwal Posyandu</span>
                </a>
                <a href="{{ route('user.notifikasi.index') }}" class="smooth-route group flex items-center gap-3.5 px-3 py-3.5 rounded-xl {{ user_nav_active('user.notifikasi.*') }}">
                    <i class="fas fa-envelope-open-text w-6 text-center text-[1.15rem] {{ user_nav_icon('user.notifikasi.*') }}"></i>
        ...
        @if($isOrangTua || $isRemaja || $isLansia || $isIbuHamil)
            <div>
                <p class="px-3 text-[11px] font-black text-slate-400 uppercase tracking-wider mb-3">Kesehatan Keluarga</p>
                <div class="space-y-1.5">
                    
                    @if($isIbuHamil)
                        <a href="{{ route('user.ibu-hamil.index') }}" class="smooth-route group flex items-center gap-3.5 px-3 py-3.5 rounded-xl {{ user_nav_active('user.ibu-hamil.*') }}">
                    ...
                    @if($isOrangTua)
                        <a href="{{ route('user.balita.index') }}" class="smooth-route group flex items-center gap-3.5 px-3 py-3.5 rounded-xl {{ user_nav_active('user.balita.*') }}">
                        ...
                        <a href="{{ route('user.imunisasi.index') }}" class="smooth-route group flex items-center gap-3.5 px-3 py-3.5 rounded-xl {{ user_nav_active('user.imunisasi.*') }}">
                            <i class="fas fa-shield-virus w-6 text-center text-[1.15rem] {{ user_nav_icon('user.imunisasi.*') }}"></i>
                            <span class="text-sm tracking-wide">Riwayat Imunisasi</span>
                        </a>
                    @endif

                    @if($isRemaja)
                        <a href="{{ route('user.remaja.index') }}" class="smooth-route group flex items-center gap-3.5 px-3 py-3.5 rounded-xl {{ user_nav_active('user.remaja.*') }}">
                        ...
                        <a href="{{ route('user.konseling.index') }}" class="smooth-route group flex items-center gap-3.5 px-3 py-3.5 rounded-xl {{ user_nav_active('user.konseling.*') }}">
                            <i class="fas fa-comments-medical w-6 text-center text-[1.15rem] {{ user_nav_icon('user.konseling.*') }}"></i>
                            <span class="text-sm tracking-wide">Tanya Bidan (Chat)</span>
                        </a>
                    @endif

                    @if($isLansia)
                        <a href="{{ route('user.lansia.index') }}" class="smooth-route group flex items-center gap-3.5 px-3 py-3.5 rounded-xl {{ user_nav_active('user.lansia.*') }}">
        ...
        @else
            <div class="mx-3 mt-4 p-5 bg-orange-50 border border-orange-200 rounded-2xl shadow-sm">
                ...
                <a href="{{ route('user.profile.edit') }}" class="flex items-center justify-center gap-2 w-full py-2.5 bg-orange-500 text-white text-xs font-bold rounded-xl hover:bg-orange-600 transition-colors shadow-sm">
        ...
        <div>
            ...
            <div class="space-y-1.5">
                <a href="{{ route('user.riwayat.index') }}" class="smooth-route group flex items-center gap-3.5 px-3 py-3.5 rounded-xl {{ user_nav_active('user.riwayat.*') }}">
                    <i class="fas fa-notes-medical w-6 text-center text-[1.15rem] {{ user_nav_icon('user.riwayat.*') }}"></i>
                    <span class="text-sm tracking-wide">Rekam Medis Keluarga</span>
                </a>
                <a href="{{ route('user.profile.edit') }}" class="smooth-route group flex items-center gap-3.5 px-3 py-3.5 rounded-xl {{ user_nav_active('user.profile.*') }}">
                    <i class="fas fa-user-cog w-6 text-center text-[1.15rem] {{ user_nav_icon('user.profile.*') }}"></i>

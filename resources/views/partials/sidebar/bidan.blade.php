...
<a href="{{ route('bidan.konseling.index') }}" class="smooth-route group flex items-center gap-3 px-3 py-3 rounded-xl {{ request()->routeIs('bidan.konseling.*') ? 'bg-sky-50 text-sky-700 font-bold border-r-4 border-sky-500' : 'text-slate-500 font-semibold hover:bg-slate-50 hover:text-sky-600' }}">
    <i class="fas fa-comments-medical w-5 text-center text-lg {{ request()->routeIs('bidan.konseling.*') ? 'text-sky-600' : 'text-slate-400 group-hover:text-sky-500' }}"></i>

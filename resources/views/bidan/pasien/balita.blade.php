...
@section('page-subtitle', 'Monitoring tumbuh kembang & kesehatan balita')
...
<div class="card border-0 shadow-sm rounded-4">
    <div class="card-header bg-white py-3 d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
        ...
        <div class="d-flex gap-2 align-items-center">
            <a href="{{ route('bidan.laporan.balita') }}" class="btn btn-sm btn-outline-danger d-flex align-items-center gap-2">
    ...
    <div class="card-body p-0">
        <div id="dataContainer">
            @include('bidan.pasien.partials.table_balita')
        </div>
    </div>
</div>
...
$(document).ready(function() {
    ...
    function fetchData(query = '', url = null, showLoading = true) {
        if(!url) url = "{{ route('bidan.pasien.balita') }}";

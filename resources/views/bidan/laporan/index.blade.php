...
@section('title', 'Laporan Medis Posyandu')
@section('page-name', 'Rekapitulasi EMR')
...
<div class="max-w-[1250px] mx-auto animate-slide-up pb-16">
    ...
    {{-- =================================================================
         2. LAYOUT GRID (PERFECTLY ALIGNED)
         ================================================================= --}}
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-stretch">
        
        {{-- =============================================================
             PANEL KIRI: FILTER KONTROL (Menerapkan h-full)
             ============================================================= --}}
        <div class="lg:col-span-4">
            <div class="nexus-card p-8 h-full flex flex-col">
                ...
                {{-- Form dibuat flex-col agar tombol submit bisa didorong ke paling bawah --}}
                <form action="{{ route('bidan.laporan.index') }}" method="GET" class="flex flex-col flex-1">
                    <div class="space-y-6">
                        <div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="jenis" class="nexus-select border-cyan-100 bg-cyan-50/30 text-cyan-800 focus:border-cyan-400 focus:ring-cyan-50">
                                    <option value="semua" {{ $jenis == 'semua' ? 'selected' : '' }}>Semua Layanan (Terpadu)</option>
                                    <option value="balita" {{ $jenis == 'balita' ? 'selected' : '' }}>Kesehatan Anak & Balita</option>
                                    <option value="ibu_hamil" {{ $jenis == 'ibu_hamil' ? 'selected' : '' }}>Ibu Hamil (Kandungan)</option>
                                    <option value="remaja" {{ $jenis == 'remaja' ? 'selected' : '' }}>Kesehatan Remaja</option>
                                    <option value="lansia" {{ $jenis == 'lansia' ? 'selected' : '' }}>Lansia (Manula)</option>
                                </select>
                    ...
                    {{-- mt-auto mendorong div ini ke paling bawah agar sejajar dengan panel kanan --}}
        ...
        {{-- =============================================================
             PANEL KANAN: PREVIEW STATS & DOWNLOAD
             ============================================================= --}}
        <div class="lg:col-span-8 flex flex-col gap-6 lg:gap-8">
            
            {{-- Kartu Pratinjau Analitik (Metrics) --}}
            <div class="nexus-card p-8 md:p-10 flex-1 flex flex-col justify-center">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                    <div>
                        ...
                        <p class="text-[12px] font-bold text-slate-500 uppercase tracking-widest">Periode Laporan: <span class="text-cyan-600">{{ $periode->translatedFormat('F Y') }}</span></p>
                    </div>
                    <span class="inline-flex px-4 py-2 bg-indigo-50 text-indigo-600 text-[10px] font-black uppercase tracking-widest rounded-[12px] border border-indigo-100 items-center gap-2 shadow-sm">
                        <i class="fas fa-tags text-indigo-400"></i> {{ str_replace('_', ' ', $jenis) }}
                    </span>
                </div>

                {{-- Kotak Angka Berdimensi --}}
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 md:gap-5 mb-8">
                    <div class="stat-box bg-slate-50 hover:bg-white hover:border-slate-200 border border-slate-100 p-5 rounded-[24px] text-center shadow-[0_4px_10px_rgba(0,0,0,0.02)]">
                        <span class="block text-[34px] font-black text-slate-800 font-poppins leading-none mb-2">{{ $ringkasan['total'] }}</span>
                    ...
                    <div class="stat-box bg-cyan-50 hover:bg-white hover:border-cyan-200 border border-cyan-100 p-5 rounded-[24px] text-center shadow-[0_4px_10px_rgba(6,182,212,0.05)]">
                        <span class="block text-[34px] font-black text-cyan-600 font-poppins leading-none mb-2">{{ $ringkasan['balita'] }}</span>
                    ...
                    <div class="stat-box bg-pink-50 hover:bg-white hover:border-pink-200 border border-pink-100 p-5 rounded-[24px] text-center shadow-[0_4px_10px_rgba(236,72,153,0.05)]">
                        <span class="block text-[34px] font-black text-pink-600 font-poppins leading-none mb-2">{{ $ringkasan['ibu_hamil'] }}</span>
                    ...
                    <div class="stat-box bg-emerald-50 hover:bg-white hover:border-emerald-200 border border-emerald-100 p-5 rounded-[24px] text-center shadow-[0_4px_10px_rgba(16,185,129,0.05)]">
                        <span class="block text-[34px] font-black text-emerald-600 font-poppins leading-none mb-2">{{ $ringkasan['lansia'] }}</span>
                        <span class="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Lansia</span>
                    </div>
                </div>

                {{-- Kotak Peringatan Dini --}}
                <div class="flex flex-col sm:flex-row sm:items-center gap-5 p-6 rounded-[24px] bg-rose-50/50 border border-rose-100">
                    ...
                    <div>
                        ...
                        <p class="text-[13px] font-bold text-slate-700 leading-relaxed">
                            Berdasarkan EMR yang tervalidasi bulan ini, terdeteksi <strong class="text-rose-600 px-1.5 bg-rose-100 rounded-md">{{ $ringkasan['stunting'] }} Kasus Stunting/Gizi Buruk</strong> dan <strong class="text-rose-600 px-1.5 bg-rose-100 rounded-md">{{ $ringkasan['hipertensi'] }} Kasus Hipertensi</strong>.
                        </p>
                    </div>
                </div>
            </div>

            {{-- Tombol Aksi Unduh PDF --}}
            @if($ringkasan['total'] > 0)
                <form action="{{ route('bidan.laporan.cetak') }}" method="GET" class="w-full" target="_blank">
                    ...
                    <button type="submit" onclick="showLoader('MENGENERATE PDF...', 2000)" class="w-full p-6 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white rounded-[32px] shadow-[0_15px_35px_rgba(6,182,212,0.3)] hover:shadow-[0_20px_40px_rgba(6,182,212,0.4)] hover:-translate-y-1 transition-all duration-300 flex flex-col sm:flex-row items-center justify-center gap-5 group border border-cyan-400/50">
                        ...
                        <div class="text-center sm:text-left">
                            ...
                            <span class="block text-[12px] font-medium text-cyan-100">Format standar Puskesmas. Tervalidasi dengan nama Bidan ({{ Auth::user()->name }}).</span>
                        </div>
                    </button>
                </form>
            @else

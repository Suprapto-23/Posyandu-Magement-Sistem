...
@section('page-subtitle', 'Generate dan cetak laporan pemeriksaan posyandu')
...
<div class="card border-0 shadow-sm rounded-4 mb-4">
    ...
    <div class="card-body">
        <form method="GET" action="{{ route('bidan.laporan.index') }}" class="row g-3 align-items-end" id="formFilter">
            <div class="col-md-3">
...
<div class="row g-3 mb-4">
    <div class="col-12 d-flex justify-content-between align-items-center mb-2">
        <h6 class="text-dark fw-bold mb-0">
            Ringkasan Data — {{ Carbon\Carbon::createFromDate($tahun, $bulan, 1)->translatedFormat('F Y') }}
            @if($jenis !== 'semua') 
                <span class="badge bg-primary ms-2">{{ ucfirst($jenis) }}</span>
            @endif
        </h6>
    </div>
    ...
    <div class="col-6 col-md-3">
        <div class="card border-0 shadow-sm rounded-4 h-100 border-bottom border-{{ $color }} border-3">
            <div class="card-body d-flex align-items-center gap-3 p-3">
                <div class="rounded-circle bg-{{ $color }} bg-opacity-10 p-3 flex-shrink-0">
                    <i class="fas fa-{{ $icon }} text-{{ $color }} fa-lg"></i>
                </div>
                <div>
                    <div class="fw-bold fs-4 lh-1 text-dark">{{ $nilai }}</div>
                    <div class="text-muted mt-1" style="font-size:0.75rem; font-weight: 500;">{{ $label }}</div>
                </div>
            </div>
        </div>
    </div>
...
<div class="card border-0 shadow-sm rounded-4">
    <div class="card-header bg-white border-bottom d-flex justify-content-between align-items-center rounded-top-4 py-3">
        <span class="fw-bold text-dark">
            ...
            Preview Dokumen ({{ $data->count() }} Data)
        </span>
        
       {{-- TOMBOL CETAK UTAMA --}}
        <form action="{{ route('bidan.laporan.cetak') }}" method="GET" target="_blank" class="m-0 p-0">
            <input type="hidden" name="bulan" value="{{ $bulan ?? date('n') }}">
            <input type="hidden" name="tahun" value="{{ $tahun ?? date('Y') }}">
            <input type="hidden" name="kategori" value="{{ $jenis ?? 'semua' }}">
            
            <button type="submit" class="btn btn-success btn-sm px-3 shadow-sm rounded-pill" {{ isset($data) && $data->count() == 0 ? 'disabled' : '' }}>
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="small">{{ \Carbon\Carbon::parse($item->tanggal_periksa)->format('d/m/Y') }}</td>
                        <td class="fw-bold text-dark small">{{ $item->nama_pasien }}</td>
                        <td>
                            @php
                        ...
                        <td class="small text-muted">
                            BB: <span class="text-dark fw-semibold">{{ $item->berat_badan ?? '-' }} kg</span><br>
                            TB: <span class="text-dark fw-semibold">{{ $item->tinggi_badan ?? '-' }} cm</span>
                        </td>
                        <td>
                            @if(in_array(strtolower($item->status_gizi), ['stunting', 'buruk', 'risiko', 'bahaya']))
                                <span class="badge bg-danger">{{ strtoupper($item->status_gizi) }}</span>
                            @else
                                <span class="text-capitalize small fw-semibold">{{ $item->status_gizi ?? '-' }}</span>
                            @endif
                        </td>
                        <td class="small" style="max-width: 200px;">
                            @if($item->diagnosa)
                                <div class="text-truncate" title="{{ $item->diagnosa }}">
                    ...
                    <tr>
                        <td colspan="7" class="text-center py-5">
                            ...
                            <p class="text-muted small mb-0">Tidak ada data pemeriksaan {{ $jenis == 'semua' ? '' : $jenis }} pada periode ini.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>

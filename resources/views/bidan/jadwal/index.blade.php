            ...
            <table class="w-full text-left border-collapse min-w-[1050px]">
                ...
                <tbody>
                    ...
                    <tr class="nexus-table-row">
                        {{-- NO --}}
                        <td class="px-6 py-4 text-[13px] font-semibold text-slate-400 align-middle text-center">{{ $jadwals->firstItem() + $index }}</td>
                        ...
                        <td class="px-6 py-4 align-middle">
                            <div class="flex items-center gap-4">
                                <div class="date-card w-12 h-12 rounded-[14px] bg-slate-50 border border-slate-200 flex flex-col items-center justify-center shrink-0">
                                    <span class="text-[9px] font-bold text-cyan-600 uppercase tracking-widest">{{ \Carbon\Carbon::parse($jadwal->tanggal)->translatedFormat('M') }}</span>
                                    <span class="text-[18px] font-black text-slate-800 leading-none mt-0.5">{{ \Carbon\Carbon::parse($jadwal->tanggal)->format('d') }}</span>
                                </div>
                                <div>
                                    <p class="font-bold text-slate-800 text-[14px] mb-1 font-poppins">{{ \Carbon\Carbon::parse($jadwal->tanggal)->translatedFormat('l, Y') }}</p>
                                    <p class="text-[11px] font-semibold text-slate-500 flex items-center gap-1.5">
                                        <i class="far fa-clock text-cyan-500"></i> {{ date('H:i', strtotime($jadwal->waktu_mulai)) }} - {{ date('H:i', strtotime($jadwal->waktu_selesai)) }}
                                    </p>
                                </div>
                            </div>
                        </td>
                        ...
                        {{-- RINCIAN KEGIATAN --}}
                        <td class="px-6 py-4 align-middle">
                            <p class="font-bold text-slate-800 text-[14.5px] mb-1 font-poppins text-wrap leading-tight">{{ $jadwal->judul }}</p>
                            <p class="text-[12px] font-medium text-slate-500 line-clamp-1 mb-1.5 max-w-[260px]">{{ $jadwal->deskripsi ?? 'Tidak ada deskripsi tambahan.' }}</p>
                            <p class="text-[11px] font-semibold text-slate-600 flex items-center gap-1.5"><i class="fas fa-map-marker-alt text-rose-400"></i> {{ $jadwal->lokasi }}</p>
                        </td>
                        ...
                        <td class="px-6 py-4 align-middle">
                            <div class="flex flex-col items-start gap-2">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-indigo-50 text-indigo-700 text-[10px] font-bold rounded-lg border border-indigo-100 uppercase tracking-widest">
                                    <i class="fas fa-tags text-indigo-400 text-[9px]"></i> {{ $jadwal->kategori }}
                                </span>
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 text-emerald-700 text-[10px] font-bold rounded-lg border border-emerald-100 uppercase tracking-widest">
                                    <i class="fas fa-bullseye text-emerald-400 text-[9px]"></i> {{ str_replace('_', ' ', $jadwal->target_peserta) }}
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-center align-middle">
                            ...
                            @endphp
                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-bold uppercase tracking-widest border {{ $statusConf[0] }}">
                                <i class="fas {{ $statusConf[2] }} {{ $jadwal->status == 'aktif' ? 'animate-pulse' : '' }}"></i> {{ $statusConf[1] }}
                            </span>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right align-middle">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('bidan.jadwal.edit', $jadwal->id) }}" class="smooth-route w-9 h-9 rounded-[10px] bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-amber-50 hover:text-amber-500 hover:border-amber-200 transition-all border border-transparent" title="Edit Jadwal">

...
<div id="smoothLoader" class="fixed inset-0 bg-slate-50/90 backdrop-blur-md z-[9999] flex flex-col items-center justify-center transition-all duration-300 opacity-0 pointer-events-none">
    <div class="relative w-20 h-20 flex items-center justify-center mb-5">
        ...
        <i class="fas fa-save text-amber-600 text-2xl animate-pulse"></i>
    </div>
    <p class="text-amber-800 font-poppins font-black tracking-widest text-[11px] animate-pulse" id="loaderText">MENYIMPAN PERUBAHAN...</p>
</div>
...
<div class="max-w-[800px] mx-auto animate-slide-up pb-10">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-slate-500 mt-1.5 font-medium text-[13px] sm:text-sm">Perubahan status atau lokasi akan langsung memperbarui database.</p>
        </div>
        <a href="{{ route('bidan.jadwal.index') }}" class="smooth-route inline-flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 text-slate-600 font-bold text-[13px] rounded-xl hover:bg-slate-50 hover:text-amber-600 transition-colors shadow-sm shrink-0">
    ...
    <div class="bg-white rounded-[32px] border border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden flex flex-col">
        ...
        <div class="px-8 py-6 border-b border-slate-100 bg-amber-50/50 flex items-center gap-3">
            <div class="w-10 h-10 rounded-xl bg-amber-100 text-amber-600 flex items-center justify-center text-lg"><i class="fas fa-pen-nib"></i></div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    <div>
                        ...
                        <div class="relative group-focus-within">
                            ...
                            <input type="date" name="tanggal" value="{{ old('tanggal', $jadwal->tanggal) }}" required class="premium-input cursor-pointer" style="padding-left: 46px; padding-right: 16px;">
                        </div>
                    </div>
                    <div>
                        <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-2 pl-1">Waktu Mulai <span class="text-rose-500">*</span></label>
                        <div class="relative group-focus-within">
                            <i class="far fa-clock input-icon"></i>
                            <input type="time" name="waktu_mulai" value="{{ old('waktu_mulai', date('H:i', strtotime($jadwal->waktu_mulai))) }}" required class="premium-input px-4 cursor-pointer text-center">
                        </div>
                    </div>
                    <div>
                        <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-2 pl-1">Waktu Selesai <span class="text-rose-500">*</span></label>
                        <div class="relative group-focus-within">
                            <i class="far fa-clock input-icon"></i>
                            <input type="time" name="waktu_selesai" value="{{ old('waktu_selesai', date('H:i', strtotime($jadwal->waktu_selesai))) }}" required class="premium-input px-4 cursor-pointer text-center">
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="relative group-focus-within">
                            ...
                            <select name="status" required class="premium-input cursor-pointer appearance-none border-amber-200 bg-amber-50 focus:bg-white text-amber-800" style="padding-right: 16px;">

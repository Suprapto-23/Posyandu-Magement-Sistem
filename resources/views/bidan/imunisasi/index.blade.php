...
<div class="max-w-7xl mx-auto animate-slide-up">
    ...
    <div class="bg-white rounded-[24px] border border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.03)] overflow-hidden">
        ...
        <div class="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <form method="GET" action="{{ route('bidan.imunisasi.index') }}" class="relative w-full md:w-80">
                <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama atau vaksin..." class="w-full bg-white border border-slate-200 rounded-xl pl-11 pr-4 py-2.5 text-[13px] font-medium focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100 outline-none transition-all shadow-sm">
            </form>
        </div>
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td class="py-4 px-6 align-middle">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-bold text-slate-800 text-[13px]">{{ \Carbon\Carbon::parse($imun->tanggal_imunisasi)->translatedFormat('F Y') }}</p>
                                    <p class="text-[11px] font-medium text-slate-400 mt-0.5"><i class="fas fa-clock mr-1"></i> {{ $imun->created_at->format('H:i') }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="py-4 px-6 align-middle">
                            ...
                            @endphp
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-full bg-{{$bCol}}-50 text-{{$bCol}}-500 flex items-center justify-center shrink-0 border border-{{$bCol}}-100">
                                    <i class="fas fa-{{$bIcon}} text-xs"></i>
                                </div>
                                <div>
                                    <p class="font-extrabold text-slate-700 text-[13px] mb-0.5 truncate max-w-[180px]">{{ $namaPasien }}</p>
                                    <span class="text-[9px] font-black text-{{$bCol}}-500 uppercase tracking-wider">{{ $tipePasien }}</span>
                                </div>
                            </div>
                        </td>

                        <td class="py-4 px-6 align-middle">
                            <p class="text-[13px] font-black text-slate-800 mb-1"><i class="fas fa-prescription-bottle-alt text-cyan-500 mr-1.5"></i> {{ $imun->vaksin }}</p>
                            <span class="inline-flex items-center gap-1.5 px-2 py-0.5 bg-slate-50 border border-slate-100 text-slate-500 text-[10px] font-bold rounded-md">
                                Dosis: <strong class="text-cyan-600">{{ $imun->dosis }}</strong> &middot; {{ $imun->jenis_imunisasi }}
                            </span>
                        </td>
                        ...
                        <td class="py-4 px-6 align-middle">
                            <div class="flex items-center gap-2">
                                <div class="w-7 h-7 rounded-full bg-cyan-600 text-white flex items-center justify-center text-[10px] font-bold shadow-inner shrink-0">
                                    {{ strtoupper(substr($imun->kunjungan->petugas->name ?? 'S', 0, 1)) }}
                                </div>
                                <span class="text-[12px] font-bold text-slate-600 truncate max-w-[120px]">{{ $imun->kunjungan->petugas->name ?? 'Sistem' }}</span>
                            </div>
                        </td>
                        ...
                        <td class="py-4 px-6 text-center align-middle">
                            <div class="flex items-center justify-center gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('bidan.imunisasi.show', $imun->id) }}" class="smooth-route w-8 h-8 rounded-lg bg-white border border-slate-200 text-slate-400 flex items-center justify-center hover:text-cyan-600 hover:border-cyan-300 hover:bg-cyan-50 shadow-sm transition-all" title="Detail">
                                    <i class="fas fa-eye text-xs"></i>
                                </a>
                                <a href="{{ route('bidan.imunisasi.edit', $imun->id) }}" class="smooth-route w-8 h-8 rounded-lg bg-white border border-slate-200 text-slate-400 flex items-center justify-center hover:text-amber-500 hover:border-amber-300 hover:bg-amber-50 shadow-sm transition-all" title="Edit">
                                    <i class="fas fa-pen text-xs"></i>
                                </a>
                                <form action="{{ route('bidan.imunisasi.destroy', $imun->id) }}" method="POST" class="m-0" onsubmit="return confirm('Hapus permanen data medis ini?')">
                                    @csrf @method('DELETE')

...
<div class="max-w-[1400px] mx-auto animate-slide-up pb-10">
    ...
    {{-- KONTROL PENCARIAN & FILTER TABS --}}
    {{-- BUG FIX: Menghapus overflow-hidden agar dropdown bisa tembus ke bawah --}}
    <div class="bg-white rounded-[32px] border border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.03)] flex flex-col mb-6 relative z-[50]">
        <div class="p-5 sm:p-6 bg-slate-50/50 rounded-[32px] flex flex-col gap-5">
            ...
            <div class="flex flex-wrap items-center gap-2.5">
                ...
                @foreach($tabs as $t)
                    <a href="{{ route('bidan.rekam-medis.index', ['type' => $t['id']]) }}" class="smooth-route px-5 py-2.5 rounded-xl text-[12px] font-black uppercase tracking-widest transition-all flex items-center gap-2 border {{ $type == $t['id'] ? 'bg-'.$t['col'].'-500 text-white border-'.$t['col'].'-600 tab-active' : 'bg-white text-slate-500 border-slate-200 hover:border-'.$t['col'].'-300 hover:text-'.$t['col'].'-600 shadow-sm' }}">
                        <i class="fas {{ $t['icon'] }} text-lg"></i> <span>{{ $t['label'] }}</span>
                    </a>
                @endforeach
            </div>

            {{-- FORM OMNI-SEARCH (Pencarian Prediktif Cerdas) --}}
            <form action="{{ route('bidan.rekam-medis.index') }}" method="GET" class="w-full relative" id="omniSearchContainer">
                ...
                <input type="text" name="search" id="omniSearchInput" value="{{ request('search') }}" autocomplete="off" placeholder="Ketik nama pasien yang ingin dicari (Lalu tekan Enter)..." class="w-full pl-12 pr-12 py-4 bg-white border-2 border-slate-200 rounded-[16px] text-[14px] font-bold text-slate-700 focus:border-cyan-500 focus:ring-4 focus:ring-cyan-50 outline-none transition-all shadow-sm">
                
                {{-- Tombol Submit Tersembunyi untuk Enter Key --}}
                <button type="submit" class="hidden"></button>

                @if(request('search'))
                    <a href="{{ route('bidan.rekam-medis.index', ['type' => $type]) }}" class="absolute inset-y-0 right-4 flex items-center text-rose-400 hover:text-rose-600 transition-colors">
                        <i class="fas fa-times-circle text-lg"></i>
                    </a>
                @endif

                {{-- Dropdown Prediktif (Akan melayang bebas sekarang) --}}
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
        ...
        <div class="patient-card bg-white rounded-[24px] border border-slate-200/80 shadow-sm p-6 flex flex-col relative overflow-hidden group">
            ...
            <div class="absolute -right-8 -top-8 w-32 h-32 {{ $bgC }} rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10"></div>

            <div class="flex items-start gap-4 mb-6 z-10">
                <div class="w-14 h-14 rounded-[16px] flex items-center justify-center text-2xl shrink-0 {{ $bgC }} {{ $txC }} border {{ $bdC }} shadow-inner group-hover:bg-cyan-500 group-hover:text-white group-hover:border-cyan-600 transition-colors duration-300">
                    <i class="fas {{ $icn }}"></i>
                </div>
                
                <div class="flex-1 min-w-0 pt-1">
                    <h4 class="font-black text-slate-800 text-[16px] mb-1 group-hover:text-cyan-700 transition-colors truncate font-poppins">{{ $pasien->nama_lengkap }}</h4>
                    <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest"><i class="far fa-id-card"></i> {{ $pasien->nik ?? 'TANPA NIK' }}</p>
                </div>
            </div>
            ...
            <div class="mt-auto z-10 space-y-4">
                <div class="bg-slate-50 p-3 rounded-xl border border-slate-100 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-[12px] font-bold text-slate-700">{{ \Carbon\Carbon::parse($pasien->tanggal_lahir)->translatedFormat('d M Y') }} <span class="text-cyan-600 ml-1">({{ \Carbon\Carbon::parse($pasien->tanggal_lahir)->age }} Thn)</span></p>
                    </div>
                </div>
        ...
        <div class="col-span-full text-center py-20 px-4 bg-white rounded-[32px] border border-slate-200 shadow-sm">
            ...
            <p class="text-[13px] font-medium text-slate-500 max-w-sm mx-auto">Pasien "<span class="text-cyan-600 font-bold">{{ request('search') }}</span>" tidak ditemukan di kategori ini. Coba klik kategori lain di bawah kolom pencarian.</p>
        </div>
        @endforelse
    </div>
    ...
    searchInput.addEventListener('input', function() {
        ...
        if (val.length > 0) {
            ...
            debounceTimer = setTimeout(() => {
                fetch(`{{ route('bidan.rekam-medis.index') }}?search=${encodeURIComponent(val)}`, {
                    headers: {
                ...
                .then(data => {
                    ...
                    if (data.length === 0) {
                        ...
                        data.forEach(pasien => {
                            ...
                            let link = "{{ route('bidan.rekam-medis.show', ['pasien_type' => ':type', 'pasien_id' => ':id']) }}";
